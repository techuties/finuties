...
**New in this review (2026-07-30):** St. Louis Fed FRED `fredgraph.csv` series downloads (no API
key), CBOE CDN historical VIX/VVIX/SKEW JSON, OCC daily options volume CSV (params pinned),
UK ONS beta dataset API + CPIH CSV download (~4.7 MB), IMF SDMX 3.0 CPI data CSV
(`CPI/5.0.0`), Central Bank of the Republic of Turkey (TCMB) daily FX XML, Bulgarian National
Bank FX XML (promoted from P1; TLS OK), NOAA CO-OPS water levels + station catalog, Open-Meteo
forecast and air-quality APIs, USGS NWIS instantaneous values, NY Fed Survey of Consumer
Expectations workbook, OurAirports airports CSV, and data.gov.uk CKAN package search. Prior P0
anchors re-verified live (Canada SEMA, UK Sanctions CSV, GLEIF, ESMA FIRDS, FINRA CNMS
2026-07-29, TreasuryDirect, BoC Valet, FDIC, Federal Register, BNM, OpenFEMA, USGS monitored
volcanoes via `hans-public`). **BanRep Colombia still offline** (maintenance HTML). Swiss SECO
still XHTML interstitial; Destatis guest still HTML shell; MAS directory still 404; Stats NZ
root still 404; USGS `vsc/api/volcanoApi/*` paths 404 — use `hans-public` only.
...
- **Implementation:** daily sync of OHLC close; upsert by index symbol + date; start with `_VIX`,
  then VVIX/SKEW. Note some alternate CBOE CDN paths return S3 AccessDenied.
...
- **Verified:** `reportDate=20260728` returned ~5.0 MB / ~152k CSV rows.
...
- **Entry point:**
  `GET https://www.bnb.bg/Statistics/StExternalSector/StExchangeRates/StERForeignCurrencies/index.htm?downloadFunc=xml&search=&search_and_or=OR&search_period_from=&search_period_to=&search_period_type=period&download=xml`
- **Docs:** https://www.bnb.bg/Statistics/StExternalSector/StExchangeRates/index.htm
- **Access:** no authentication; XML `ROWSET/ROW` (~7 KB; first row is Bulgarian header labels —
  skip `F_ORDER=0`). Verified 2026-07-30 with standard TLS (USD rate present for 29.07.2026).
...
| Cleveland Fed inflation / yield-curve nowcasts | Fed indicator pages | Useful companions to GDPNow/ADS/SCE; durable XLSX/CSV URLs still not pinned (404 guesses on 2026-07-30). |
...
2. **US markets microstructure & banking:** TreasuryDirect auctions + FINRA Reg SHO short volume +
   OCC options volume + CBOE VIX/VVIX/SKEW + FRED `fredgraph.csv` allowlist + OFR + Chicago Fed
   NFCI + Philadelphia Fed ADS + Atlanta Fed GDPNow + NY Fed SCE + FDIC BankFind + CFPB
   complaints.
...
4. **Global FX prints:** RBA + Norges Bank + Riksbank + SNB + BCB PTAX + NBP + CNB + Danmarks
   ...
   CBAR Azerbaijan + BNM Malaysia + TCMB Türkiye + BNB Bulgaria (+ BanRep Colombia when restored;
...
and downloaded the response body on 2026-07-30 UTC (prior 2026-07-29 notes retained where still
accurate).
...
| FINRA CNMS short volume (2026-07-29) | 200 | pipe text ~536 KB |
...
| IMF SDMX 3.0 structure dataflow *(new)* | 200 | JSON; 222 dataflows |
| IMF SDMX 3.0 CPI `5.0.0` CSV *(new)* | 200 | SDMX-CSV |
...
| BanRep consultaMercadoCambiario | 200 | **maintenance HTML** (still offline) |
...
| Stats NZ open data root | 404 | JSON not-found |
