...
**New in this review (2026-08-09):** Philadelphia Fed Manufacturing Business Outlook Survey (MBOS)
CSV history/diffusion indexes; Kansas City Fed Manufacturing and Services Survey historical
workbooks; APRA Monthly Authorised Deposit-taking Institution statistics XLSX (+ back-series);
Bank of Japan Main Time-series Statistics English CSVs (FX, policy rate, monetary base, money
stock); IRENASTAT PxWeb JSON API (renewable electricity capacity/generation); TWSE OpenAPI
exchange-report JSON (indexes, PE/PB/yield, daily averages, ex-dividend calendar); JPX listed-issue
/ equity-statistics workbooks; US Census New Residential Construction `newresconst.xlsx`; and
JRC EDGAR 2024 GHG booklet XLSX. **Path / contract notes:** Coin Metrics Community API now rejects
`limit` — use `page_size` (BTC PriceUSD **2026-08-08** ~64907); EIA `STEO_m.xlsx` still live on full
GET (~1.09 MB) but HTTP Range requests can 404 — download without Range; FINRA CNMS `20260808`
AccessDenied (weekend / non-session — keep T+1 business-day discovery); FHFA monthly
`hpi_master.csv` still anonymous (~17 MB) under #44. **Re-verified:** GLEIF ~3,397,891 LEIs
(golden copy **2026-08-08**); UK Sanctions List report date still **06-Aug-2026**; CISA KEV
catalogVersion **2026.08.07** (1,662); OTC Markets screener ~18,085; FINRA CNMS 20260807; OCC
volume-query 20260807 ~5.1 MB; Fear & Greed **31** Fear; NY Fed SOFR 2026-08-06 3.65%;
OpenSanctions ~291.2k entities (updated **2026-08-09**); FCA/AFM net shorts; Swiss SECO XML list
date still **2026-07-30**; Polymarket + Kalshi; HKEX List of Securities; EIA WPSR companion XLS.
**Still blocked / watch:** MarineCadastre AIS ZIP GETs still 404; OpenSky connection failed;
OpenNEM power stats still 401; GIE AGSI/ALSI need `x-key`; NASA FIRMS needs `MAP_KEY`; USDA NASS /
Ember API / Electricity Maps / Banxico / OpenAQ / AirNow key-gated or Cloudflare-blocked; Metaculus
403; BaFin/FI Sweden/FSMA/CONSOB short-selling machine exports not stably extracted (HTML
registers); Dallas Fed TMOS / Richmond Fed survey workbook URLs still not pinned; Baker Hughes
stable workbook still undiscovered; Destatis guest REST still HTML app shell.
...
| Dallas Fed TMOS / Richmond Fed surveys | regional Fed survey pages | Pages live; stable historical workbook URLs still not pinned (404 on prior path guesses). |
...
1. **Compliance and entity identity:** Canada SEMA + UK Sanctions + Swiss SECO (#191) + GLEIF +
   ...
   + TWSE OpenAPI (#203) + JPX listed issues (#204) + PRH Finland + Brreg Norway + Companies House
   free data product + LittleSis + Federal Register + SIPRI Milex (geopolitical spend context).
2. **US markets microstructure & banking:** TreasuryDirect auctions + FINRA Reg SHO short volume +
   ...
   NFCI/CFNAI + Philadelphia Fed ADS + Philadelphia Fed SPF (#179) + Philadelphia Fed MBOS (#197)
   + Atlanta Fed GDPNow + Atlanta Fed Wage Growth + NY Fed SCE + NY Fed Empire State (#193) + NY
   Fed GSCPI + Dallas Trimmed Mean PCE + Cleveland inflation nowcast + SF Fed news sentiment +
   SF Fed Proxy Funds Rate (#178) + KC Fed LMCI (#195) + KC Fed Manufacturing/Services (#198/#199)
   + FDIC BankFind (+ SOD deposits API) + APRA Monthly ADI (#200) + CFPB complaints + IAPD +
   BrokerCheck.
...
6. **Power and gas:** Elexon + ENTSOG + MIDAS + Energinet + Elia + REE Spain + RTE éco2mix (#192)
   ...
   Energy-Charts DE power/price (#162) + EPA eGRID (#175) + EDGAR GHG booklet (#206) (+ AEMO
   NEMWEB only after terms clearance; OpenNEM power still auth-gated).
...
and downloaded the response body on **2026-08-09 UTC** (prior 2026-08-08/07/06/05/04/03/02/01/07-31
notes retained where still accurate).

| Source | HTTP | Response type / observation |
|---|---:|---|
| Philly Fed MBOS `bos_dif.csv` / `bos_history.csv` *(new)* | 200 | CSV ~78 KB / ~578 KB |
