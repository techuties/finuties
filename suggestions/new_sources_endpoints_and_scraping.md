...
**New in this review (2026-08-15):** remaining Federal Reserve DDP ZIP packages on the
same G.17/H.15 contract — H.3 aggregate reserves, G.20 finance companies, E.2 Survey of
Terms of Business Lending, Policy Rates (`PRATES`), SCOOS dealer-financing terms, and
household Debt Service Ratios (`DSR`); Census Time Series / Trend Charts zipped CSVs
(`econ_getzippedfile/?programCode=`) which finally pin **MARTS** (promoted from P1) plus
QSS, QFR, MWTS, MRTS, BFS, and VIP; NCUA 5300 Call Report quarterly ZIP
(`call-report-data-2026-03.zip`, ~7.8 MB). **Allowlist extensions:** FRED `fredgraph.csv`
adds `TOTRESNS` / `BOGMBASE` / `IORB` / `DFEDTARU` / `DFEDTARL` / `FINCP` / `TOTALSL` /
`WRESBAL` as companions to the new Fed prints; `TDSP` now also companions native DSR
(#236). **Re-verified:** GLEIF ~3,403,130 LEIs (golden copy **2026-08-14**); UK Sanctions
List report date **14-Aug-2026**; CISA KEV catalogVersion **2026.08.14** (1,665); FINRA
CNMS 20260814 / 20260813 live (~538–540 KB); OCC volume-query 20260814 / 20260813 ~4.8 MB
with `format=csv`; Fear & Greed **34** Fear; Coin Metrics BTC PriceUSD **2026-08-14**
~62925; NY Fed SOFR 2026-08-13 3.62%; OpenSanctions ~291.7k (updated **2026-08-15**);
Swiss SECO list date **2026-08-13**; CFTC TFF/CIT Socrata latest **2026-08-11**; PredictIt
197 markets; SGX 893 indices; Malaysia CPI / Census M3 / Polymarket / Kalshi / Canada SEMA
still live. **Still blocked / watch:** OTC Markets screener HTTP/2 reset this run (keep
#120 with backoff); Richmond Fed manufacturing workbook URLs still 404; MarineCadastre AIS
ZIP GETs still 404; OpenSky unreachable; OpenNEM power 401; CFTC Bank Participation is
HTML-only (August 2026 page live — keep P1); Atlanta Fed BIE native XLS still HTML
interstitial (use FRED `CORESTICKM159SFRBATL`); Stats NZ ADE API is subscription-key
gated; GIE/FIRMS/NASS/Ember/Banxico/OpenAQ/AirNow/KOSIS key-gated or Cloudflare-blocked;
Metaculus 403; BaFin/FI Sweden/FSMA/CONSOB short-selling exports still unpinned; Baker
Hughes workbook undiscovered; Destatis guest REST HTML/405; RBNZ unavailable; MAS paths
404; Bank of Korea ECOS needs a free API key.
...
- **Access:** no authentication; ZIP ~320 KB updated **2026-08-03**; `VIP-mf.csv` ~1.7 MB /
  84,272 lines.
- **Implementation:** same Time Series ZIP parser. Same-catalog housing companions to fold
  after the first adapter (do not stand up parallel parsers): `RESSALES` (new home sales,
  ~103 KB, 2026-07-24), `HV` (vacancies/homeownership, ~55 KB, 2026-07-28), `MHS2`
  (manufactured housing, ~28 KB, 2026-08-06), `RESCONST` (complements #205 XLSX), `QTAX`
  (state & local taxes), `QPR` (public pensions), `MTIS` (manufacturing & trade inventories,
  ~102 KB, 2026-08-14), `M3ADV` / `FTDADV` (advance M3 / goods trade).
...
- **Terms/risk:** NCUA attribution; multi-file schema drift each cycle — keep `AcctDesc`
  versioned with the quarter. June 2026 ZIP still unpublished as of 2026-08-15.
...
| NCUA 5300 Call Report quarterly ZIP | `https://ncua.gov/files/publications/analysis/call-report-data-YYYY-MM.zip` | **Promoted to P0 #244** (`2026-03` live; `2026-06` not yet published). |
...
2. **US markets microstructure & banking:** TreasuryDirect auctions + FINRA Reg SHO short volume +
   ...
   FDIC BankFind (+ SOD deposits API) + NCUA Call Report (#244) + APRA Monthly ADI (#200) +
   CFPB complaints + IAPD + BrokerCheck.
3. **Financial conditions and macro stats:** Fed H.15 packages (#166) + FRED `fredgraph.csv`
   ...
   Time Series MARTS/QSS/QFR/MWTS/MRTS/BFS (#237–#242).
...
5. **Commodities, energy, housing, credit:** World Bank Pink Sheet + IMF Primary Commodity Prices +
   ...
   construction spending (#243) + Census MARTS/MRTS retail (#237/#241) + ANBIMA IMA (#185) +
   BCB Focus (#186) + FRED Case-Shiller/UMCSENT/ICSA/HOUST/TOTRESNS/IORB allowlist companions.
...
and downloaded the response body on **2026-08-15 UTC** (prior 2026-08-14/11/09/08/07/06/05/04/03/02/01/07-31
notes retained where still accurate).

| Source | HTTP | Response type / observation |
|---|---:|---|
| Fed H.3 DDP ZIP *(new)* | 200 | ZIP ~554 KB; `H3_data.xml` ~6.6 MB |
