...
**New in this review (2026-08-11):** Dallas Fed regional survey historical workbooks finally
pinned (TMOS `alldata`/`index` XLS; TSSOS/TROS service+retail; Banking Conditions Survey;
Dallas Energy Survey; Agricultural Survey) — promoted from the long-standing P1 watch list;
PredictIt and Manifold Markets anonymous prediction-market JSON (complements Polymarket/Kalshi);
SGX public indices JSON (~893 indices); Malaysia `api.data.gov.my` CPI/IPI/monetary-aggregates
catalog JSON; US Census M3 Manufacturers' Shipments `table1a.xlsx`; Federal Reserve G.17
Industrial Production & Capacity Utilization SDMX ZIP package. **Allowlist extensions:** FRED
`fredgraph.csv` adds `STLFSI4` / `INDPRO` / `TCU` / `AMTMNO`; ECB Data Portal (#83) adds `CISS`
(systemic stress) and `MIR` (MFI interest rates). **Re-verified:** GLEIF ~3,398,874 LEIs (golden
copy **2026-08-11**); UK Sanctions List report date still **06-Aug-2026**; CISA KEV
catalogVersion **2026.08.10** (1,662); OTC Markets screener ~18,080; FINRA CNMS 20260810 (~542 KB;
20260811/09/08 AccessDenied — keep T+1 business-day discovery); OCC volume-query 20260810 ~5.1 MB;
Fear & Greed **29** Fear; Coin Metrics BTC PriceUSD **2026-08-10** ~63908; NY Fed SOFR 2026-08-07
3.62%; OpenSanctions ~291.3k entities (updated **2026-08-11**); Swiss SECO XML list date
**2026-08-11** (moved from 2026-07-30); Polymarket + Kalshi; Canada SEMA XML. **Still blocked /
watch:** MarineCadastre AIS ZIP GETs still 404; OpenSky connection failed; OpenNEM power stats
...
403; BaFin/FI Sweden/FSMA/CONSOB short-selling machine exports not stably extracted; Richmond Fed
manufacturing workbook URLs still 404; Baker Hughes stable workbook still undiscovered; Destatis
guest REST still HTML/405; RBNZ unavailable; MAS paths 404; Bank of Korea ECOS requires free API
key (`sample` demo key works — not zero-credential).
...
2. **US markets microstructure & banking:** TreasuryDirect auctions + FINRA Reg SHO short volume +
   ...
   Fed GSCPI + Dallas Trimmed Mean PCE + Dallas Fed TMOS/TSSOS/BCS/DES/AgSurvey (#207–#211) +
   Cleveland inflation nowcast + SF Fed news sentiment + SF Fed Proxy Funds Rate (#178) + KC Fed
   LMCI (#195) + KC Fed Manufacturing/Services (#198/#199) + Fed G.17 IP package (#217) + Census
   M3 (#216) + FDIC BankFind (+ SOD deposits API) + APRA Monthly ADI (#200) + CFPB complaints +
   IAPD + BrokerCheck.
...
and downloaded the response body on **2026-08-11 UTC** (prior 2026-08-09/08/07/06/05/04/03/02/01/07-31
notes retained where still accurate).

| Source | HTTP | Response type / observation |
|---|---:|---|
| Dallas Fed TMOS `alldata.xls` / `alldata_sa.xls` *(new)* | 200 | XLSX ~482 KB / ~237 KB |
