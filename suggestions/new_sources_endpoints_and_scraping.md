...
**New in this review (2026-08-23):** seven credential-free official prints
that sit outside the Census vintage cluster added yesterday — **IRS SOI
county-to-county migration 2022–23** (`countyinflow2223.csv` ~4.57 MB /
90k+ rows; Autauga County US+foreign inflow 2,148 returns / 4,413
exemptions / $138.8m AGI; LM **2026-03-19**); **IRS SOI ZIP-code AGI
2022** (`22zpallagi.csv` ~216 MB / 166,132 lines; LM **2025-02-14**);
**FEC 2026 campaign-finance bulk** (`weball26.zip` ~193 KB → 4,290
candidate summary rows, refreshed **2026-08-23 05:43**; `cn26.zip` 8,429
candidate master rows); **BTS border crossing** Socrata `keg4-3bc2`
(275,901 rows through **2026-07**; Laredo personal vehicles 443,419);
**DOL ETA 539 weekly UI claims by state** (`ar539.csv` ~13.34 MB; through
**2026-08-15**; LM **2026-08-22 19:50**); **BEA county GDP and personal
income** (`CAGDP1.zip` ~1.93 MB, `CAINC1.zip` ~3.47 MB; LM **2026-02-05**;
CAINC1 through **2024**); and **Census LEHD QWI US**
(`qwi_us_sa_f_gn_ns_op_u.csv.gz` ~6.49 MB; LM **2026-06-26**). **PEP
county/metro companions** (`co-est2025-alldata.csv` ~2.07 MB;
`cbsa-est2025-alldata.csv` ~827 KB) fold into #273 — not new P0s.
**Allowlist extensions:** FRED `fredgraph.csv` adds `EMRATIO` / `CCSA` /
`IC4WSA` / `M2V` / `AAA` / `BAA` / `SOFR` / `COMPUTSA` / `MSPUS` /
`IPMAN` / `BUSINV` / `EXPGS` / `NETEXP` / `RECPROUSM156N` / `DFII5` /
`DFII30` / `OVXCLS` / `GEPUCURRENT` / `RSXFS` / `PPIFIS` / `DEXCHUS` /
`TB3MS`. **Re-verified:** GLEIF ~3,409,898 LEIs (golden copy
**2026-08-22**, preview publish **2026-08-23**, ~3,409,946 records); UK
Sanctions List report date still **20-Aug-2026**; CISA KEV
catalogVersion **2026.08.21** (1,674); FINRA CNMS **20260821** live
(weekend **20260822** AccessDenied); OCC volume-query **20260821** CSV
live; Fear & Greed **66** Greed; Coin Metrics BTC PriceUSD **2026-08-22**
~77020 (`page_size`, not `sort=time.desc`); NY Fed SOFR **2026-08-20**
3.63%; OpenSanctions ~291.2k (updated **2026-08-23**); PredictIt 190;
SGX 897; Dallas Fed WEI XLSX unchanged (LM **2026-08-20**; FRED `WEI`
**2026-08-15** = **2.56**); CFTC TFF/CIT still **2026-08-18**; Malaysia
CPI / NCUA March ZIP still live. **Still blocked / watch:** NCUA
`call-report-data-2026-06.zip` still 404; `cbp24*` still 404; NES
`data/2024/` empty; SUSB `tables/2023/` now **404** (was WAF-rejected
2026-08-22); ABS `data/2024/` still CI/DFT canaries; Census ACES latest
full tables still **2022**; QPC Q2 2026 still unpublished; `FinFutWk.txt`
still Cloudflare 403; FRED `SAHM` 404 and `TEDRATE` ends **2022-01** —
do not add; BTS monthly on-time ZIP is live but multi-10-MB — keep P1;
ACS table-based-SF guessed 2023/2024 dirs 404.
...
- **Terms/risk:** quarterly LEHD release. Census / LED attribution.
  LODES commute files on the same host are a later same-family add, not
  a parallel P0.
...
2. **US markets microstructure & banking:** TreasuryDirect auctions + FINRA Reg SHO short volume +
   ...
   county GDP/income (#279) + LEHD QWI (#280) + NCUA Call Report (#244)
...
3. **Financial conditions and macro stats:** Fed H.15 packages (#166) + FRED `fredgraph.csv`
   ...
   PPIFIS / DEXCHUS / TB3MS allowlist companions.
...
5. **Commodities, energy, housing, credit:** World Bank Pink Sheet + IMF Primary Commodity Prices +
   ...
   (#278) + BEA county GDP/income (#279) + LEHD QWI (#280) + ANBIMA IMA
...
and downloaded the response body on **2026-08-23 UTC** (prior 2026-08-22/21/20/19/18/17/15/14/11/09/08/07/06/05/04/03/02/01/07-31
