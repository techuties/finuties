...
**New in this review (2026-08-24):** four credential-free official prints
that sit outside yesterday's IRS/FEC/BTS/DOL/BEA/LEHD cluster —
**BLS QCEW no-key CSV API** (2025 Q1–Q4 live; US Q4 **12,428,415**
establishments / month-3 employment **156,700,218** / avg weekly wage
**$1,569**; Autauga Q1 1,160 establishments; industry-10 nationwide slice
~3.83 MB); **IRS EO Business Master File** (`eo1.csv` ~48.6 MB / 278,015
lines through `eo3.csv` ~164.6 MB / 955,287 lines; LM **2026-08-10**;
`eo_xx` foreign ~417 KB); **Senate LDA lobbying API** (no key;
**1,976,319** filings, **56,003** in 2026, latest post
**2026-08-23 23:44 ET**; 17,452 registrants / 136,347 clients /
674,034 contribution reports); and **CMS Open Payments 2025 CSVs**
(promoted from P1 — research file ~897 MB, general ~9.23 GB, profile
~405 MB; LM **2026-06-02**; `download.cms.gov/openpayments/...` now
pinned). **Folds:** BEA `CAGDP9.zip` (~15.4 MB) and `CAINC30.zip`
(~23.5 MB) into #279. **Allowlist extensions:** FRED `AWHMAN` /
`NEWORDER` / `USPRIV` / `IMPGS` / `EXPGSC1` / `TWEXBGSMTH` / `DEXJPUS` /
`DEXUSUK`. Do not add `IORAL` (404). **Re-verified:** FinUties `/health`
online and `/api/v1/status/p0` **green** 8/8; GLEIF ~3,409,981 LEIs
(golden copy **2026-08-23**; preview publish **2026-08-24**, ~3,409,997);
UK Sanctions List report date still **20-Aug-2026**; CISA KEV
catalogVersion **2026.08.21** (1,674); FINRA CNMS **20260821** live
(weekend **20260822**/**20260823** AccessDenied); OCC **20260821** CSV
live (**20260822** rejected as future); Fear & Greed **73** Greed; Coin
Metrics BTC PriceUSD **2026-08-23** ~77593; NY Fed SOFR still
**2026-08-20** 3.63%; OpenSanctions ~291.2k (updated **2026-08-24**);
CFTC TFF/CIT still **2026-08-18**; PredictIt / SGX 897 / Malaysia CPI /
Dallas Fed WEI (LM **2026-08-20**; FRED `WEI` **2026-08-15** = **2.56**)
still live; FEC `weball26.zip` refreshed **2026-08-24 05:51**; DOL ETA
539 LM **2026-08-23 19:50** (still through **2026-08-15**); BTS border
count still **275,901**. **Still blocked / watch:** NCUA
`call-report-data-2026-06.zip` still 404; `cbp24*` still 404; NES
`data/2024/` empty; SUSB `tables/2023/` still WAF-rejected; QPC Q2 2026
still unpublished; `FinFutWk.txt` Cloudflare 403; IRS `2324` /
`23zpallagi` 404; OCC derivatives Q2 2026 404; BLS LAUS
`www.bls.gov` / `download.bls.gov` 403 from this environment (use QCEW
API, not LAUS HTML); HUD FMR guessed XLSX returned empty 202 / API 401;
GovInfo collections require an API key; FRED `IORAL` 404.
...
2. **US markets microstructure & banking:** TreasuryDirect auctions + FINRA Reg SHO short volume +
   ...
   CMS Open Payments (#284) + NCUA Call Report (#244)
...
5. **Commodities, energy, housing, credit:** World Bank Pink Sheet + IMF Primary Commodity Prices +
   ...
   (#278) + BEA county GDP/income (#279) + LEHD QWI (#280) + BLS QCEW
   (#281) + IRS EO BMF (#282) + ANBIMA IMA
...
8. **Cyber, physical, climate, and country risk:** CISA KEV + FIRST EPSS + OSV.dev + CIRCL CVE +
   ...
   SAHIE county insurance (#272) + CMS Open Payments (#284) + IMF PortWatch
...
and downloaded the response body on **2026-08-24 UTC** (prior 2026-08-23/22/21/20/19/18/17/15/14/11/09/08/07/06/05/04/03/02/01/07-31
