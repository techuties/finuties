...
**New in this review (2026-08-07):** SF Fed Proxy Funds Rate workbook, Philadelphia Fed Survey
of Professional Forecasters median-growth XLSX, Polymarket Gamma/CLOB public markets, Kalshi
public trade API (markets/events/exchange status), Penn World Table productivity workbook,
adsb.lol open ADS-B (military + lat/lon radius), UN Operational Rates of Exchange XML (~223
currency prints), ANBIMA IMA Brazilian fixed-income index workbook, BCB Focus/Olinda market
expectations OData (IPCA/Selic), CEPALSTAT/ECLAC indicator JSON API, and SEC Form D quarterly
TSV ZIPs (path under `/files/datastandardsinnovation/` for 2026q2). FRED allowlist extended with
`MEDCPIM158SFRBCLE`, `CORESTICKM159SFRBATL`, `ICSA`, and `JTSJOL`. USGS volcano entry extended
with `getElevatedVolcanoes`. **Re-verified:** GLEIF ~3,396,479 LEIs (golden copy 2026-08-07),
UK Sanctions List report date **06-Aug-2026**, CISA KEV catalogVersion **2026.08.06** (1,661),
OTC Markets screener ~18,079, USDM current shapefile map date **2026-08-04**, NHC quiet-valid,
PortWatch chokepoints, FINRA CNMS 20260806 ~537 KB, OCC volume-query 20260806 ~5.0 MB, CBOE
`_VIX.json` timestamp 2026-08-07, Coin Metrics BTC `PriceUSD` 2026-08-06 ~64212, Fear & Greed
value **29** (Fear), Energy-Charts DE power, OpenSanctions ~291.6k entities (updated
2026-08-07), NY Fed SOFR/averages still live on Markets API. **Still blocked / watch:** CIRCL.lu
anonymous burst → HTTP 429; ReliefWeb v1 **410**, v2 approved `appname` still required; GFW SQL
needs free API key; NASA FIRMS needs `MAP_KEY`; USDA NASS Quick Stats unauthorized without key;
Banxico/KOSIS/Ember/Electricity Maps key-gated; Metaculus API auth-required (403); CBOE
`_MOVE.json` AccessDenied; Swiss SECO XHTML interstitial; Australia DFAT fetch failed;
MarineCadastre AIS ZIP GETs still 404; OpenSky connection failed; Bybit country-blocked;
Binance.com geo-451 (use Binance.US); PHMSA pipeline ZIP Access Denied from this environment.
...
- **Access:** Community tier works without an API key for documented public metrics (verified
  `PriceUSD` for BTC/ETH on 2026-08-05). Some metrics/frequencies remain paid — allowlist only
...
### 178. SF Fed Proxy Funds Rate *(new)*
...
- **Entry point:**
  `GET https://www.frbsf.org/wp-content/uploads/sites/4/proxy-funds-rate-data.xlsx` (~28 KB XLSX
  verified 2026-08-07)
- **Docs:** https://www.frbsf.org/economic-research/indicators-data/proxy-funds-rate/
- **Access:** no authentication; Excel workbook.
- **Implementation:** checksum workbook; parse date + proxy-rate columns; retain publication
  vintage; daily/weekly poll with skip-unchanged.
...
|---|---|---|
| NOAA MarineCadastre AIS bulk | `https://coast.noaa.gov/htdata/CMSP/AISDataHandler/2024/` (and `/2023/`) | Year indexes list `AIS_YYYY_MM_DD.zip`, but direct ZIP GETs returned 404 here on 2026-08-03 — rediscover working object URLs / mirror before maritime AIS ingest. |
...
2. **US markets microstructure & banking:** TreasuryDirect auctions + FINRA Reg SHO short volume +
   SEC CNS fails-to-deliver (#165) + SEC Form D quarters (#188) + OCC options volume
   (`reportType=D`) + CBOE VIX/VVIX/SKEW (historical charts path) + OTC Markets www screener +
   TIC major foreign holders + Fed H.15 DDP packages (#166) + FRED `fredgraph.csv` allowlist
   (incl. claims/JOLTS/median & sticky CPI) + OFR + Chicago Fed NFCI/CFNAI + Philadelphia Fed ADS
   + Philadelphia Fed SPF (#179) + Atlanta Fed GDPNow + Atlanta Fed Wage Growth + NY Fed SCE +
   NY Fed GSCPI + Dallas Trimmed Mean PCE + Cleveland inflation nowcast + SF Fed news sentiment +
   SF Fed Proxy Funds Rate (#178) + FDIC BankFind + CFPB complaints + IAPD + BrokerCheck.
...
4. **Global FX prints:** RBA + Norges Bank + Riksbank + SNB + BCB PTAX + NBP + CNB + Danmarks
   ...
   of Moldova + National Bank of Georgia + UN Operational Rates (#184) (+ rediscovered HKMA ER when available).
5. **Commodities, energy, housing, credit:** World Bank Pink Sheet + IMF Primary Commodity Prices +
   LBMA metals + EIA WPSR/Brent/HH/NGS-history/STEO/EPM/DPR files + FHFA HPI + Freddie Mac PMMS +
   UK Land Registry + CSO Ireland HPA02 + Census FT-900 + ANBIMA IMA (#185) + BCB Focus (#186) +
   FRED Case-Shiller/UMCSENT/ICSA allowlist companions.
6. **Power and gas:** Elexon + ENTSOG + MIDAS + Energinet + Elia + REE Spain + ERCOT dashboards
   + CAISO demand/fuel CSVs (#167) + NYISO public CSVs + IESO Ontario GenOutputCapability +
   UK Carbon Intensity + Energy-Charts DE power/price (#162) + EPA eGRID (#175)
...
8. **Cyber, physical, climate, and country risk:** CISA KEV + FIRST EPSS + OSV.dev + CIRCL CVE +
   ...
   (#180/#181) + Penn World Table (#182) + CEPALSTAT (#187) + UN SDG + ILOSTAT labour allowlists.
...
and downloaded the response body on **2026-08-07 UTC** (prior 2026-08-06/05/04/03/02/01/07-31 notes
retained where still accurate).

| Source | HTTP | Response type / observation |
|---|---:|---|
| SF Fed Proxy Funds Rate XLSX *(new)* | 200 | XLSX ~28 KB |
| Philadelphia Fed SPF `medianGrowth.xlsx` *(new)* | 200 | XLSX ~144 KB |
| Polymarket Gamma markets/events *(new)* | 200 | JSON; active markets + events |
| Polymarket CLOB `/markets` *(new)* | 200 | JSON ~1.8 MB catalog |
| Kalshi markets/events/status *(new)* | 200 | JSON; exchange_active true |
| Penn World Table `pwt100.xlsx` *(new)* | 200 | XLSX ~6.6 MB |
| adsb.lol `/v2/mil` + JFK radius *(new)* | 200 | JSON ~80 KB / ~16 KB |
| UN Operational Rates XML *(new)* | 200 | XML ~48 KB; 223 currency rows |
| ANBIMA `ima_completo.xls` *(new)* | 200 | Excel ~252 KB |
| BCB Focus Olinda IPCA/Selic *(new)* | 200 | OData JSON |
| CEPALSTAT thematic-tree + indicator 2202 *(new)* | 200 | JSON ~375 KB / ~671 KB |
| SEC Form D `2026q2_d.zip` *(new)* | 200 | ZIP ~3.8 MB → TSV bundle |
| SEC Form D `2026q1_d.zip` *(recheck path)* | 200 | ZIP ~3.6 MB |
| FRED `ICSA` / `JTSJOL` / median & sticky CPI *(allowlist)* | 200 | CSV |
| USGS `getElevatedVolcanoes` *(extended)* | 200 | JSON ~3.3 KB |
| GLEIF lei-records *(recheck)* | 200 | JSON:API; ~3,396,479 LEIs; golden copy 2026-08-07 |
| UK Sanctions List CSV *(recheck)* | 200 | CSV ~49.6 MB; Report Date **06-Aug-2026** |
| CISA KEV *(recheck)* | 200 | JSON; catalogVersion **2026.08.06**; count 1,661 |
| OTC Markets stock-screener *(recheck)* | 200 | JSON; count ~18,079 |
| USDM current shapefile ZIP *(recheck)* | 200 | ZIP; members `USDM_20260804.*` |
| NHC `CurrentStorms.json` *(recheck)* | 200 | JSON; `activeStorms: []` |
| PortWatch Daily_Chokepoints_Data *(recheck)* | 200 | JSON features |
| FINRA CNMS 20260806 | 200 | pipe text ~537 KB |
| OCC volume-query 20260806 | 200 | CSV ~5.0 MB |
| CBOE `_VIX.json` | 200 | JSON; timestamp 2026-08-07 |
| Coin Metrics `PriceUSD` BTC | 200 | JSON; 2026-08-06 ~64212 |
| Fear & Greed `limit=1` | 200 | JSON; latest value 29 (Fear) |
| Energy-Charts `public_power?country=de` | 200 | JSON |
| OpenSanctions sanctions index | 200 | JSON; ~291.6k entities; updated 2026-08-07 |
| NY Fed SOFR last/1 | 200 | JSON; effectiveDate 2026-08-05; 3.64% |
| ECB €STR `EST` sample | 200 | CSV |
| Metaculus questions API | 403 | auth required |
| SEC `cnsfails202607a.zip` | 200 | ZIP ~1.2 MB → pipe text; settlement 20260701+ |
| Fed H.15 Fed funds/prime/discount package *(new)* | 200 | CSV ~1.9 KB; `RIFSPFF_N.WW` etc. |
| Fed H.15 weekly averages package *(new)* | 200 | CSV ~12 KB; CP + bills/CMTs |
| Fed H.15 Treasury CMT package *(new)* | 200 | CSV ~981 KB; 1m–30y |
| CAISO `demand.csv` / `fuelsource.csv` *(promoted)* | 200 | CSV ~6 KB / ~0.9 KB |
| openFDA drug/device/food enforcement *(promoted)* | 200 | JSON; drug enforcement total ~17,860 |
| openFDA NDC + FAERS *(promoted)* | 200 | JSON |
| NASA POWER daily point *(promoted)* | 200 | JSON Feature; T2M/PRECTOTCORR |
| NASA DONKI CME / FLR *(new)* | 200 | JSON ~364 KB / ~37 KB |
| PSMSL `filelist.txt` + `1.rlrdata` *(new)* | 200 | text ~139 KB / ~68 KB |
| CPC `oni.ascii.txt` / `sstoi.indices` *(new)* | 200 | text ~23 KB / ~39 KB |
| GHCN-Daily `USW00094728.csv` *(new)* | 200 | CSV ~17.8 MB |
| NHTSA recalls Tesla Model 3 2023 *(promoted)* | 200 | JSON; Count=11 |
| EPA `egrid2023_data_rev2.xlsx` *(new)* | 200 | XLSX ~21.2 MB |
| Blockchain.com `stats` + charts *(new)* | 200 | JSON; BTC ~$64.8k on probe |
| CPSC SaferProducts recalls YTD *(promoted)* | 200 | JSON ~1.1 MB |
| USDM dataservices national CSV *(recheck)* | 404 | prefer shapefile/GeoJSON paths |
| USDM current GeoJSON *(prior)* | 200 | GeoJSON ~19.1 MB |
| NASA GISS `GLB.Ts+dSST.csv` | 200 | CSV ~13 KB |
| NSIDC NH sea-ice extent v4 | 200 | CSV |
| Hyperliquid `meta` POST | 200 | JSON ~17.5 KB |
| World Bank Pink Sheet monthly | 200 | XLSX |
| IMF `External_Data.xls` PCPS | 200 | OOXML XLSX despite `.xls` |
| FRED `fredgraph.csv?id=DGS10` | 200 | CSV ~268 KB |
| CBOE `_VVIX.json` / `_SKEW.json` | 200 | JSON history |
| CBOE `_MOVE.json` | 403 | AccessDenied |
| ISO 10383 MIC CSV | 200 | CSV ~492 KB |
| deps.dev npm `lodash` | 200 | JSON ~20 KB |
| NOAA GML `co2_mm_mlo.txt` | 200 | text ~60 KB |
| CIRCL.lu CVE-2024-3400 | 429 | Too Many Requests (`20 per 1 minute`) — pace #158 |
| NASA FIRMS without MAP_KEY | 400 | Invalid MAP_KEY |
| USDA NASS Quick Stats without key | 401 | unauthorized |
| Ember Energy API without key | 403 | No API key set |
| Electricity Maps without token | 401 | Invalid auth-token |
| Banxico SIE without token | 400 | Token inválido |
| ReliefWeb v1 disasters | 410 | API v1 decommissioned |
| Bybit v5 tickers | 403 | CloudFront country block |
