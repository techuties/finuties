...
**New in this review (2026-08-17):** remaining distinctive Census Time Series ZIPs that
were folded as companions under VIP (#243) — **RESSALES** new home sales, **HV** housing
vacancies, **MTIS** manufacturing & trade inventories, **QTAX** state & local taxes, and
**MHS2** current manufactured housing; NY Fed Household Debt and Credit Q2 2026 workbook
(`hhd_c_report_2026q2.xlsx`, released 2026-08-11); FDIC BankFind institution **financials**
JSON (bank-level quarterly assets/deposits/equity/income on the same no-key host as #85);
Treasury TIC **SLT** tables 1–2 (foreign holdings of US long-term securities and the
mirror US holdings of foreign long-term securities); Philadelphia Fed **NBOS**
nonmanufacturing history workbook (services counterpart to MBOS #197). Fed DDP is now
exhausted — the `default.htm` package list matches the existing #166/#217/#220–#236
allowlist (`rel=H16` / `G5` / `E16` / `FA` still 400). **Allowlist extensions:** FRED
`fredgraph.csv` adds `HSN1F` / `RRVRUSQ156N` / `ISRATIO` / `PERMIT` / `BAMLH0A0HYM2` /
`T10YIE` / `MORTGAGE30US` / `DTWEXBGS` / `EXHOSLUSM495S`; NY Fed SCE Credit Access
workbook folds into #113. **Re-verified:** GLEIF ~3,403,779 LEIs (golden copy
**2026-08-17**); UK Sanctions List report date **14-Aug-2026**; CISA KEV catalogVersion
**2026.08.14** (1,665); FINRA CNMS 20260814 live (20260815 Saturday AccessDenied); OCC
volume-query 20260814 CSV live; Fear & Greed **31** Fear; Coin Metrics BTC PriceUSD
**2026-08-16** ~62818; NY Fed SOFR still **2026-08-13** 3.62% (Monday 08:00 UTC, before
the 08:00 ET print); OpenSanctions ~292.4k (updated **2026-08-17**); Swiss SECO list date
**2026-08-17**; CFTC TFF/CIT Socrata latest **2026-08-11**; PredictIt 197; SGX 893;
Malaysia CPI / Census M3 / Polymarket / Kalshi / Canada SEMA still live. **Still blocked
/ watch:** OTC Markets screener timed out again (keep #120 with backoff); NCUA
`call-report-data-2026-06.zip` still 404; Richmond Fed manufacturing workbooks still 404;
...
2. **US markets microstructure & banking:** TreasuryDirect auctions + FINRA Reg SHO short volume +
   ...
   FDIC BankFind (+ SOD deposits API) + FDIC institution financials (#251) + TIC SLT (#252) +
   Philadelphia Fed NBOS (#253) + NCUA Call Report (#244) + APRA Monthly ADI (#200) +
...
and downloaded the response body on **2026-08-17 UTC** (prior 2026-08-15/14/11/09/08/07/06/05/04/03/02/01/07-31
notes retained where still accurate).

| Source | HTTP | Response type / observation |
|---|---:|---|
| Census `programCode=RESSALES` *(new)* | 200 | ZIP ~105 KB; `RESSALES-mf.csv` ~531 KB / 29,238 lines; LM **2026-08-17 04:58** |
