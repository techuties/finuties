...
**New in this review (2026-08-08):** FCA UK daily net short-positions XLSX, AFM Netherlands net
short-positions CSV export, Swiss SECO consolidated sanctions XML (~40 MB, previously interstitial),
RTE France éco2mix Opendatasoft JSON (promoted from P1), NY Fed Empire State Manufacturing Survey
CSVs, US Census Building Permits Survey state annual TXT, Kansas City Fed LMCI workbook, and NESO
(GB) demand open-data CKAN CSV/datastore. **Path fixes:** SF Fed Proxy Funds Rate now under
`/wp-content/uploads/proxy-funds-rate.xlsx` (+ `-data.xlsx` / chart CSV); UN Operational Rates
confirmed on `xsql2XML.php` / `xsql2CSV.php`; ANBIMA IMA on `/ima/arqs/ima_completo.xls`; BCB Focus
entity names are plural `ExpectativasMercadoAnuais` / `ExpectativasMercadoSelic` (plus
`ExpectativaMercadoMensais`); CBOE keep `delayed_quotes/charts/historical/_VIX|_VVIX|_SKEW.json`
(`us_indices/daily_prices/_*.json` still AccessDenied); USDM shapefile path
`shapefiles_m/USDM_current_M.zip` (map date still **2026-08-04**). **Re-verified:** GLEIF
~3,397,167 LEIs (golden copy 2026-08-07); UK Sanctions List report date **06-Aug-2026**; CISA KEV
catalogVersion **2026.08.07** (1,662); OTC Markets screener ~18,085; FINRA CNMS 20260807; OCC
volume-query 20260807 ~5.1 MB; Fear & Greed **30** Fear; Coin Metrics BTC PriceUSD 2026-08-07
~64869; NY Fed SOFR 2026-08-06 3.65%; OpenSanctions sanctions collection ~291.2k entities (updated
2026-08-08); Polymarket Gamma + Kalshi markets; adsb.lol `/v2/mil`; CEPALSTAT thematic-tree; SEC
Form D 2026q2 ZIP; Penn World Table `pwt100.xlsx`; Energy-Charts DE power; PortWatch chokepoints;
NHC quiet-valid; ECB €STR `EST` CSV. **Still blocked / watch:** MarineCadastre AIS ZIP GETs still
404 despite directory listing; OpenSky connection failed; OpenNEM power stats now auth-required
(use NEMWEB ZIPs after terms review); GIE AGSI/ALSI need `x-key`; NASA FIRMS needs `MAP_KEY`;
USDA NASS / Ember / Electricity Maps / Banxico / OpenAQ / AirNow key-gated; Metaculus 403;
Australia DFAT fetch failed; RBNZ site-unavailable; MAS API paths 404; Baker Hughes stable
workbook still undiscovered; OpenFoodFacts / Wikimedia pageviews remain P1 enrichment.
...
| Kansas City Fed LMCI | `.../lmcicharts-070726.xlsx` | **Promoted to P0 #195**; Richmond Fed manufacturing workbook URL still not stably extracted — keep watching. |
...
1. **Compliance and entity identity:** Canada SEMA + UK Sanctions + Swiss SECO (#191) + GLEIF +
   ...
   + PRH Finland + Brreg Norway + Companies House free data product + LittleSis + Federal Register
...
2. **US markets microstructure & banking:** TreasuryDirect auctions + FINRA Reg SHO short volume +
   ...
   (#188) + OCC options volume (`reportType=D`) + CBOE VIX/VVIX/SKEW (historical charts path) +
   OTC Markets www screener + TIC major foreign holders + Fed H.15 DDP packages (#166) + FRED
   `fredgraph.csv` allowlist (incl. claims/JOLTS/median & sticky CPI) + OFR + Chicago Fed
   NFCI/CFNAI + Philadelphia Fed ADS + Philadelphia Fed SPF (#179) + Atlanta Fed GDPNow + Atlanta
   Fed Wage Growth + NY Fed SCE + NY Fed Empire State (#193) + NY Fed GSCPI + Dallas Trimmed Mean
   PCE + Cleveland inflation nowcast + SF Fed news sentiment + SF Fed Proxy Funds Rate (#178) +
   KC Fed LMCI (#195) + FDIC BankFind (+ SOD deposits API) + CFPB complaints + IAPD + BrokerCheck.
...
5. **Commodities, energy, housing, credit:** World Bank Pink Sheet + IMF Primary Commodity Prices +
   ...
   UK Land Registry + CSO Ireland HPA02 + Census FT-900 + Census BPS permits (#194) + ANBIMA IMA
   (#185) + BCB Focus (#186) + FRED Case-Shiller/UMCSENT/ICSA allowlist companions.
6. **Power and gas:** Elexon + ENTSOG + MIDAS + Energinet + Elia + REE Spain + RTE éco2mix (#192)
   + NESO GB demand (#196) + ERCOT dashboards + CAISO demand/fuel CSVs (#167) + NYISO public CSVs
   + IESO Ontario GenOutputCapability + UK Carbon Intensity + Energy-Charts DE power/price (#162)
   + EPA eGRID (#175) (+ AEMO NEMWEB only after terms clearance; OpenNEM power now auth-gated).
...
and downloaded the response body on **2026-08-08 UTC** (prior 2026-08-07/06/05/04/03/02/01/07-31
notes retained where still accurate).

| Source | HTTP | Response type / observation |
|---|---:|---|
| FCA short-positions daily XLSX *(new)* | 200 | XLSX ~3.1 MB |
