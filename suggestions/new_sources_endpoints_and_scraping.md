...
**New in this review (2026-08-22):** three credential-free Census prints that
close the remaining small-area / vintage-population gaps beside yesterday's
public-finance cluster — **Census Small Area Income and Poverty Estimates
(SAIPE) 2024** (`est24all.txt` ~847 KB + `est24all.xls` ~1.12 MB; US poverty
**40,354,267** / **12.1%**, median household income **$81,604**; LM
**2026-01-27**); **Census Small Area Health Insurance Estimates (SAHIE) 2024**
(`sahie-2024-csv.zip` ~13.65 MB → `sahie_2024.csv` ~111.6 MB; created
**23JUN26**, published August 2026, LM **2026-08-06**); and **Census Population
Estimates (PEP) 2025** national age-sex CSV (~18 KB; US total
**341,784,857**) plus state components `NST-EST2025-ALLDATA.csv` (~54 KB).
**Friday COT arrived:** CFTC TFF/CIT Socrata latest is now **2026-08-18**
(LM **2026-08-21 19:30**). `FinFutWk.txt` is Cloudflare-challenged from this
environment — keep using Socrata. Dallas Fed WEI native XLSX unchanged (LM
**2026-08-20**; through **08/15/2026**); FRED `WEI` last print **2026-08-15**
= **2.56**. **Allowlist extensions:** FRED `fredgraph.csv` adds `DGS1` /
`DGS7` / `DGS20` / `DFII10` / `MICH` / `USREC` / `DCOILBRENTEU` / `GASREGW` /
`M1SL` / `A191RL1Q225SBEA` / `BOPGSTB` / `FYFSD` / `GFDEBTN` / `W875RX1` /
`AAA10Y` / `T10YFF` / `DEXUSEU`. **Re-verified:** GLEIF ~3,409,002 LEIs
(golden copy **2026-08-21**, preview publish **2026-08-22**, ~3,409,606
records); UK Sanctions List report date still **20-Aug-2026**; CISA KEV
catalogVersion **2026.08.21** (1,674); FINRA CNMS **20260821** live; OCC
volume-query **20260821** CSV live; Fear & Greed **71** Greed; Coin Metrics
BTC PriceUSD **2026-08-21** ~78359; NY Fed SOFR **2026-08-20** 3.63%;
OpenSanctions ~291.2k (updated **2026-08-22**); PredictIt 190; SGX 897;
Malaysia CPI / NCUA March ZIP still live. **Still blocked / watch:** NCUA
`call-report-data-2026-06.zip` still 404; `cbp24*` still 404; NES `data/2024/`
empty; SUSB `tables/2023/` WAF-rejected this run (empty yesterday); ABS
`data/2024/` still CI/DFT topic XLSX canaries (fold into #262); Census ACES
latest full tables still **2022** (2023/2024 dirs only `covid/`); QPC Q2 2026
workbook still unpublished (Q1 remains current); Census Time Series leftovers
`MHS` / `M3ADV` / `FTDADV` / `MRTSADV` / `MWTSADV` remain same-adapter
companions; Cleveland Fed native `mcpi_revised.csv` Sitecore 404; TIC guessed
extras still 404; Richmond Fed manufacturing workbooks still unpinned; CBO
budget/economic data still 403 interstitial; BEA NIPA `NipaData{A,Q,M}.txt`
anonymous files are live — treat as a raw-upstream fallback for the terminal
BEA route, not a parallel P0; CFS 2022 PUMS (~678 MB) and AHS 2023 National
PUF (~129 MB) stay P1 research dumps; OCC derivatives Q2 2026 PDF/XLSX still
404 (Q1 2026 PDF live).
...
- **Implementation:** ingest the national age-sex file as the hot path;
  ...
  grain. County/metro files on the same `2020-2025/` tree can fold in
  later, not as new P0s.
...
| Census QPC Q1 2026 workbook | `.../qpc/tables/2026/2026-qtr-table-final-q1.xlsx` | **Promoted to P0 #270**. Time Series `programCode=QPC` is empty. Q2 2026 still unpublished (2026-08-22). |
...
| Census PEP 2025 vintage files | `.../popest/datasets/2020-2025/national/asrh/nc-est2025-agesex-res.csv` | **Promoted to P0 #273**. State `NST-EST2025-ALLDATA.csv` folds into #273. |
...
2. **US markets microstructure & banking:** TreasuryDirect auctions + FINRA Reg SHO short volume +
   ...
   (#273) + NCUA Call Report (#244) + APRA Monthly ADI
...
3. **Financial conditions and macro stats:** Fed H.15 packages (#166) + FRED `fredgraph.csv`
   ...
   T10YFF / DEXUSEU allowlist companions.
...
5. **Commodities, energy, housing, credit:** World Bank Pink Sheet + IMF Primary Commodity Prices +
   ...
   population (#273) + ANBIMA IMA (#185) + BCB Focus (#186) + FRED Case-Shiller/UMCSENT/ICSA/
...
8. **Cyber, physical, climate, and country risk:** CISA KEV + FIRST EPSS + OSV.dev + CIRCL CVE +
   ...
   SAHIE county insurance (#272) + IMF PortWatch
...
and downloaded the response body on **2026-08-22 UTC** (prior 2026-08-21/20/19/18/17/15/14/11/09/08/07/06/05/04/03/02/01/07-31
notes retained where still accurate).

| Source | HTTP | Response type / observation |
|---|---:|---|
| Census SAIPE `est24all.txt` *(new)* | 200 | text ~847 KB; US poverty **40,354,267** / **12.1%**; median HH **$81,604**; LM **2026-01-27** |
