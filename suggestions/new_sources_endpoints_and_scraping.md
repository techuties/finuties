...
**New in this review (2026-08-21):** three credential-free prints that close the
remaining US public-finance / capacity gaps — **Census Annual Survey of Public
Pensions (ASPP) 2025** unit CSV (`ASPP_Unit_File_2025.csv` ~6.75 MB / 94,182
item rows; annual counterpart to quarterly QPR #264); **Census Annual Survey of
School System Finances (F-33) FY2024** (`elsec24.txt` ~8.64 MB district CSV);
and **Census Quarterly Survey of Plant Capacity Utilization Q1 2026**
(`2026-qtr-table-final-q1.xlsx` ~65 KB; full-production utilization **75.5%**).
**Current vintages** of yesterday's public-finance P0s: ASPEP **2025** ZIP
(`2025_individual_unit_files.zip` ~11.2 MB; `25empst.txt` / `25empid.txt`);
STC **FY2025** transposed XLSX (~23 KB); government finances **2024** unit ZIP
(`2024_Individual_Unit_Files.zip` ~4.27 MB; `2024FinEstDAT_07152026modp.txt`
~17.4 MB). Dallas Fed WEI native XLSX refreshed Thursday (LM **2026-08-20**,
~80 KB) through **08/15/2026**; FRED `WEI` last print **2026-08-15** = **2.56**.
Census Time Series leftovers `MHS` / `M3ADV` / `FTDADV` / `MRTSADV` / `MWTSADV`
remain same-adapter companions — do not parallelize. **Allowlist extensions:**
FRED `fredgraph.csv` adds `HOUST` / `GDPC1` / `NFCI` / `CFNAI` / `T10Y3M` /
`DFF` / `DGORDER` / `DCOILWTICO` / `VIXCLS` / `PCEC96` / `DGS3MO` / `DGS5` /
`USSTHPI` / `BAA10Y`. **Re-verified:** GLEIF ~3,407,818 LEIs (golden copy
**2026-08-20**, preview publish **2026-08-21**); UK Sanctions List report date
**20-Aug-2026**; CISA KEV catalogVersion **2026.08.20** (1,673); FINRA CNMS
**20260820** live (~540 KB); OCC volume-query **20260820** CSV live (~4.93 MB);
Fear & Greed **72** Greed; Coin Metrics BTC PriceUSD **2026-08-20** ~73071;
NY Fed SOFR **2026-08-19** 3.62%; OpenSanctions ~291.2k (updated **2026-08-21**);
CFTC TFF/CIT Socrata latest still **2026-08-11** (Friday COT print due later
today 2026-08-21 ~15:30 ET); PredictIt 192; SGX 897; Malaysia CPI / NCUA March
ZIP still live. **Still blocked / watch:** NCUA `call-report-data-2026-06.zip`
still 404; `cbp24*` still 404; NES `data/2024/` and SUSB `tables/2023/` empty;
ABS `data/2024/` has only CI/DFT topic XLSX canaries (fold into #262); Census
ACES latest full tables still **2022**; Cleveland Fed native `mcpi_revised.csv`
Sitecore 404; TIC guessed extras still 404; Richmond Fed manufacturing
workbooks still 404; MarineCadastre AIS ZIP GETs still 404; OpenSky
unreachable; OpenNEM power 401; CFTC Bank Participation HTML-only; Atlanta Fed
BIE native XLS interstitial; Stats NZ ADE subscription-key gated;
GIE/FIRMS/NASS/Ember/Banxico/OpenAQ/AirNow/KOSIS key-gated or Cloudflare-blocked;
Metaculus 403; BaFin/FI Sweden/FSMA/CONSOB shorts unpinned; Baker Hughes
workbook undiscovered; Destatis guest REST HTML/405; RBNZ unavailable; MAS
paths 404; Bank of Korea ECOS needs a free API key; CMS Open Payments metastore
is anonymous (updated **2026-08-11**) but bulk ZIP/datastore paths still
404/401 — keep P1; OPEC basket XML 403; JODI guessed ZIP 404; TSA
passenger-volumes is HTML only; V-Dem site 403 from this environment; NY Fed
CMDI / Staff Nowcast pages are JS interactives (FRED `CMDI` 404); OCC
derivatives Q2 2026 PDF/XLSX still 404 (Q1 2026 PDF live).
...
| Census QPC Q1 2026 workbook | `.../qpc/tables/2026/2026-qtr-table-final-q1.xlsx` | **Promoted to P0 #270**. Time Series `programCode=QPC` is empty. |
...
2. **US markets microstructure & banking:** TreasuryDirect auctions + FINRA Reg SHO short volume +
   ...
   (#270) + NCUA Call Report (#244) + APRA Monthly ADI
...
3. **Financial conditions and macro stats:** Fed H.15 packages (#166) + FRED `fredgraph.csv`
   ...
   PCEC96 / DGS3MO / DGS5 / USSTHPI / BAA10Y allowlist companions.
...
and downloaded the response body on **2026-08-21 UTC** (prior 2026-08-20/19/18/17/15/14/11/09/08/07/06/05/04/03/02/01/07-31
notes retained where still accurate).

| Source | HTTP | Response type / observation |
|---|---:|---|
| Census ASPP `ASPP_Unit_File_2025.csv` *(new)* | 200 | CSV ~6.75 MB / 94,183 lines; SAMPLE_YEAR **2025**; LM **2026-05-14** |
