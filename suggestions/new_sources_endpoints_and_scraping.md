...
**New in this review (2026-08-20):** five credential-free prints that close the
weekly-activity and US public-finance gaps — **Dallas Fed Weekly Economic Index**
native XLSX (`weekly-economic-index.xlsx` ~94 KB, through **2026-08-08**, 2.70;
FRED `WEI` is the CSV companion); **Census QPR / Quarterly Survey of Public Pensions**
Time Series ZIP promoted from a VIP/QTAX companion (**Q1-2026** total cash and
security holdings **$6,125,943 million**); **Census ASPEP 2024** individual-unit
employment/payroll ZIP (`2024_individual_unit_files.zip` ~10.9 MB); **Census State
Government Tax Collections FY2024** detailed transposed XLSX (~23 KB); and **Census
Annual Survey of State and Local Government Finances 2023** unit-file ZIP
(`2023_Individual_Unit_Files.zip` ~4.25 MB; `2023FinEstDAT_07152026modp.txt` ~17.3 MB,
LM **2026-08-04**). Census `RESCONST` Time Series ZIP (~359 KB) folds into New
Residential Construction #205 — do not parallelize. **Allowlist extensions:** FRED
`fredgraph.csv` adds `WEI` / `ANFCI` / `U6RATE` / `CES0500000003` / `PSAVERT` /
`GFDEGDQ188S` / `RRPONTSYD` / `WTREGEN` / `T5YIE` / `T5YIFR` / `PPIACO` /
`ALTSALES` / `JTSQUL` / `BAMLC0A0CM`. **Re-verified:** GLEIF ~3,406,584 LEIs
(golden copy **2026-08-19**, preview publish **2026-08-20**); UK Sanctions List
report date still **18-Aug-2026**; CISA KEV catalogVersion **2026.08.19** (1,671);
FINRA CNMS **20260819** now live (Wednesday session); OCC volume-query **20260819**
CSV live (~5.05 MB); Fear & Greed **62** Greed; Coin Metrics BTC PriceUSD
**2026-08-19** ~69268; NY Fed SOFR **2026-08-18** 3.65%; OpenSanctions ~290.7k
(updated **2026-08-20**); CFTC TFF/CIT Socrata latest still **2026-08-11** (Friday
COT print due 2026-08-21); PredictIt 191; SGX 897; Malaysia CPI / Census M3 /
NCUA March ZIP still live. **Still blocked / watch:** NCUA
...
from this environment; NY Fed CMDI / Staff Nowcast pages are JS interactives without a
pinned native CSV (FRED `CMDI` 404 — keep watching); OCC derivatives report is PDF-only
(guessed XML/XLSX 404).
...
- **Entry points (allowlist — extend carefully):**
  ...
  - `GET https://fred.stlouisfed.org/graph/fredgraph.csv?id=WEI` (Dallas Fed Weekly
    Economic Index; verified 2026-08-20 through **2026-08-08** 2.70 — companion to
...
- **Entry points:**
  - Native workbook:
    ...
    (~94 KB; LM **2026-08-13 14:58 GMT**; same bytes at
...
- **Access:** no authentication; XLSX with current WEI, vintage columns, and
  series weights. Last date in the workbook **08/08/2026**; FRED last print
  **2026-08-08** = **2.70**.
...
### 265. Census Annual Survey of Public Employment & Payroll (ASPEP) 2024 *(new)*
...
- **Entry point:**
  ...
  (~10.9 MB; LM **2026-04-16**)
- **Members:** `24empid.txt` (~2.4 MB; fixed-width individual units),
  `24empst.txt` (~6.8 MB; state-function), `24emp.xlsx` (~9.0 MB), plus README /
  tech-doc / disclaimer PDFs. Directory listing
  `.../apes/datasets/2024/` is live.
...
- **Verified 2026-08-20:** `24empid.txt` starts with a United States rollup row
  (`00000000000000United States...`); `24empst.txt` is fixed-width with
  suppression flags (`R`).
- **Implementation:** ingest `24empst.txt` first (state × function); then
  `24empid.txt` for unit-level; keep the XLSX as a convenience extract. Upsert
  `(year, geo, function, employment_type)`. Watch `.../apes/datasets/2025/` for
  the next vintage.
...
### 266. Census State Government Tax Collections (STC) FY2024 *(new)*
...
- **Access:** no authentication; XLSX. Directory `.../stc/tables/2024/` lists
  this file as the current detailed table. Title string:
  `Table 1. State Tax Collections Detailed Table: US and States: 2024`
  (amounts in thousands of dollars).
- **Implementation:** checksum; parse tax-type × state; upsert
  `(fiscal_year, state, tax_type)`. Prefer this transposed workbook over
  guessing additional FY2024 filenames. QTAX (#248) remains the quarterly
...
### 267. Census Annual Survey of State and Local Government Finances 2023 *(new)*
...
- **Value:** the full state-and-local finance public-use file (revenues,
  ...
  the annual cube that QTAX/STC only summarize. Highest-value new public-finance
  source after QPR/ASPEP/STC.
- **Entry points:**
  - `GET https://www2.census.gov/programs-surveys/gov-finances/tables/2023/2023_Individual_Unit_Files.zip`
    (~4.25 MB; LM **2026-08-04 21:01 GMT**)
  - Summary workbooks on the same directory: `statenlocaltqrr23.xlsx` (~12 KB)
    and `localtqrr23.xlsx` (~12 KB)
- **Members:** `2023FinEstDAT_07152026modp.txt` (~17.3 MB),
  `23statetypepu.txt` (~840 KB), `Fin_PID_2023.txt` (~3.7 MB), plus tech-doc
  and disclaimer PDFs.
- **Docs:** https://www.census.gov/programs-surveys/gov-finances.html
- **Access:** no authentication. Directory listing
  `.../gov-finances/tables/2023/` is live. `api.census.gov` government-finance
  endpoints are key-gated.
- **Verified 2026-08-20:** `2023FinEstDAT_07152026modp.txt` is a fixed-width /
  packed item file (sample row starts `01000022608519U    111991672023R`).
- **Implementation:** start with the state-type public-use TXT and the two
  summary XLSX files; add the full `2023FinEstDAT_*` item file after pinning
  the tech-doc layout. Upsert `(year, unit_id, item_code)`. Filename date
  stamps (`07152026modp`) will bump — discover from the directory listing.
...
| Census ASPEP 2024 unit files | `www2.census.gov/programs-surveys/apes/datasets/2024/2024_individual_unit_files.zip` | **Promoted to P0 #265**. |
| Census STC FY2024 detailed table | `www2.census.gov/programs-surveys/stc/tables/2024/FY2024-STC-Detailed-Table-Transposed.xlsx` | **Promoted to P0 #266**. |
| Census government finances 2023 unit ZIP | `www2.census.gov/programs-surveys/gov-finances/tables/2023/2023_Individual_Unit_Files.zip` | **Promoted to P0 #267**. |
...
2. **US markets microstructure & banking:** TreasuryDirect auctions + FINRA Reg SHO short volume +
   ...
   Census QPR pensions (#264) + Census ASPEP (#265) + Census STC (#266) +
   Census government finances (#267) + NCUA Call Report (#244) + APRA Monthly ADI
   (#200) + CFPB complaints + IAPD + BrokerCheck.
3. **Financial conditions and macro stats:** Fed H.15 packages (#166) + FRED `fredgraph.csv`
   ...
   QPR/ASPEP/STC/gov-finances (#264–#267) + Dallas Fed WEI (#263) + FRED PAYEMS /
   CIVPART / PCEPI / DSPIC96 / WEI / ANFCI / U6RATE / PSAVERT / RRPONTSYD /
   WTREGEN / T5YIE / PPIACO allowlist companions.
...
and downloaded the response body on **2026-08-20 UTC** (prior 2026-08-19/18/17/15/14/11/09/08/07/06/05/04/03/02/01/07-31
notes retained where still accurate).

| Source | HTTP | Response type / observation |
|---|---:|---|
| Dallas Fed `weekly-economic-index.xlsx` *(new)* | 200 | XLSX ~94 KB; last date **08/08/2026**; LM **2026-08-13** |
