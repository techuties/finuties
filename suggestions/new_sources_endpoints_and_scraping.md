...
**New in this review (2026-08-18):** Treasury **CSLT** continuous securities long-term ZIP
(`Publish/cslt.zip` → `cslt.json`, 4,260 monthly series from 1985, transmitted
**2026-08-13**, Last-Modified **2026-08-17 20:01 GMT** — the May 2026 public dataset that
had no pinned bulk URL yesterday); remaining live **SLT** tables **3** (foreign Treasury
holdings + net sales + valuation by country through **2026-06**) and **4** (gross
purchases/sales by security type); TIC **banking type/holder** prints `bltype.txt` /
`bctype.txt` as of **June 2026** (updated **2026-08-17**); Census **County Business
Patterns 2023** national + county ZIPs on `www2.census.gov` (2024 vintage still 404).
`slt_table5.txt` is the MFH table already covered by #138 — fold, do not parallelize.
`slt_table6.txt` is a 2023-and-earlier holdings-only companion to table 3. Country-level
`bl_globl.txt` / `bc_globl.txt` are historical archives through **2003-01** only — not
the live banking panel. **Allowlist extensions:** FRED `fredgraph.csv` adds CSLT headlines
`FORLTTOTALNET99996` / `FORLTEQTYPOS99996` / `FORTREASPOS69995` and Dallas Fed marketable
Treasury debt `MVMTD027MNFRBDAL`. **Re-verified:** GLEIF ~3,404,295 LEIs (golden copy
**2026-08-17**); UK Sanctions List report date **14-Aug-2026**; CISA KEV catalogVersion
**2026.08.17** (1,666); FINRA CNMS **20260817** live (Monday session); OCC volume-query
**20260817** CSV live; Fear & Greed **41** Fear; Coin Metrics BTC PriceUSD **2026-08-17**
~64434; NY Fed SOFR still **2026-08-14** 3.62% (Tuesday 08:00 UTC, before the 08:00 ET
print); OpenSanctions ~292.4k (updated **2026-08-18**); Swiss SECO list date
**2026-08-18**; CFTC TFF/CIT Socrata latest still **2026-08-11** (Tuesday COT not yet
printed at probe time); PredictIt 196; SGX 893; Malaysia CPI / Census M3 / NCUA March ZIP
still live. **Still blocked / watch:** NCUA `call-report-data-2026-06.zip` still 404;
Cleveland Fed native `mcpi_revised.csv` paths still Sitecore 404 (keep FRED
`MEDCPIM158SFRBCLE`); CSLT guessed `.csv`/`.xlsx`/`.txt` 404 — use the ZIP; TIC banking
country files (`bl-13005` etc.) 404; Richmond Fed manufacturing workbooks still 404;
...
but bulk ZIP/datastore paths 404/401 — keep P1; V-Dem site 403 from this environment.
...
- **Implementation:** discover latest `cbpYYus.zip` / `cbpYYco.zip`; parse national
  ...
  (do not parallelize): `cbpYYst.zip` (state) when present.
...
| NCUA 5300 Call Report quarterly ZIP | `https://ncua.gov/files/publications/analysis/call-report-data-YYYY-MM.zip` | **Promoted to P0 #244** (`2026-03` live; `2026-06` still 404 on 2026-08-17). |
...
| Census CBP 2023 ZIPs | `www2.census.gov/programs-surveys/cbp/datasets/2023/cbp23{us,co}.zip` | **Promoted to P0 #258**. `cbp24*` still 404. |
...
2. **US markets microstructure & banking:** TreasuryDirect auctions + FINRA Reg SHO short volume +
   ...
   Philadelphia Fed NBOS (#253) + Census CBP 2023 (#258) + NCUA Call Report (#244) +
...
and downloaded the response body on **2026-08-18 UTC** (prior 2026-08-17/15/14/11/09/08/07/06/05/04/03/02/01/07-31
notes retained where still accurate).

| Source | HTTP | Response type / observation |
|---|---:|---|
| TIC `cslt.zip` *(new)* | 200 | ZIP ~8.87 MB; `cslt.json` ~122 MB; 4,260 series; `transmissionDt` **2026-08-13**; LM **2026-08-17 20:01** |
