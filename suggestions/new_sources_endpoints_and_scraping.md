...
**New in this review (2026-07-31):** Chicago Fed CFNAI CSV, Dallas Fed Trimmed Mean PCE XLSX,
Cleveland Fed inflation-nowcast chart JSON (`nowcast_month` / `quarter` / `year`), San Francisco
Fed Daily News Sentiment Index (XLSX + CSV), OTC Markets stock-screener JSON (~18k OTC names),
Coinbase Exchange public ticker/candles/products, Kraken public ticker/OHLC/AssetPairs, L2Beat
scaling summary + activity JSON, SEC IAPD adviser firm/individual search, FINRA BrokerCheck firm
search, CourtListener REST search/courts (anonymous), and National Bank of Moldova dated FX XML.
**BanRep Colombia FX market REST restored** (JSON ticks live again). Prior P0 anchors re-verified
(Canada SEMA ~1.85 MB, UK Sanctions ~49.3 MB, GLEIF ~3.39M LEIs, FRED `fredgraph.csv` DGS10, BoC
Valet, USGS `hans-public` volcanoes, TCMB `today.xml`, BNB Bulgaria FX XML, FINRA CNMS
2026-07-30, OCC 2026-07-30, OpenFIGI, ESMA FIRDS Solr, NY Fed SCE, OurAirports, data.gov.uk).
**CBOE note:** `us_indices/daily_prices/_VIX|_VVIX|_SKEW.json` returns S3 `AccessDenied` (403);
prefer `delayed_quotes/charts/historical/_*.json` (still 200) and delayed spot quotes.
Swiss SECO still XHTML interstitial; Destatis guest still HTML shell; MAS directory still 404;
Stats NZ open-data root 502/404; RBNZ selected files still site-unavailable.
...
- **Entry point (verified 2026-07-31):**
  `GET https://totoro.banrep.gov.co/estadisticas-economicas/rest/consultaDatosService/consultaMercadoCambiario`
  (~63 KB JSON array of `[epoch_ms_string, rate_string]` pairs). Alternate/older
  `opendata/consultaMercadoCambiario` path may still exist — prefer the REST path above.
- **Access:** no authentication; JSON restored after 2026-07-29/30 maintenance HTML outage.
...
- **Verified:** `reportDate=20260730` returned ~4.8 MB CSV; `20260728` previously ~5.0 MB / ~152k rows.
...
- **Entry points:**
  - `GET https://www.frbsf.org/wp-content/uploads/news_sentiment_data.xlsx` (~417 KB)
  - `GET https://www.frbsf.org/wp-content/uploads/news-sentiment-chart-1.csv` (~276 KB; daily from 1980)
- **Docs:** https://www.frbsf.org/research-and-insights/data-and-indicators/daily-news-sentiment-index/
- **Access:** no authentication; XLSX + CSV.
- **Implementation:** prefer the CSV for daily ingest (`date,Daily news sentiment`); checksum;
  upsert by date; keep the workbook as a secondary full product.
...
- **Entry point:** `GET https://www.otcmarkets.com/research/stock-screener/api?page=1&pageSize=100`
  (JSON; `count` ~18088 on 2026-07-31; response may be a JSON-encoded string — parse twice if needed).
...
- **Entry point:**
  ...
  (example `date=30.07.2026` → ~7 KB XML `ValCurs` with EUR/USD/… rows).
...
| Fear & Greed / mempool.space / CoinPaprika | `https://api.alternative.me/fng/?limit=3`, `https://mempool.space/api/v1/fees/recommended` | Re-verified live 2026-07-31; useful after DeFiLlama + Coinbase/Kraken/L2Beat P0s. Confirm ToS. |
...
- **Key-required statistical APIs:** FRED **REST API** (key required — use no-key `fredgraph.csv`
  P0 #103 instead), BEA, EIA (`API_KEY_MISSING` confirmed 2026-07-26; still gated),
  Banxico SIE (token required), Ember Energy, OpenAQ v3 / AirNow, Congress.gov, GovInfo, CourtListener,
...
2. **US markets microstructure & banking:** TreasuryDirect auctions + FINRA Reg SHO short volume +
   OCC options volume + CBOE VIX/VVIX/SKEW (historical charts path) + OTC Markets screener +
   FRED `fredgraph.csv` allowlist + OFR + Chicago Fed NFCI/CFNAI + Philadelphia Fed ADS +
   Atlanta Fed GDPNow + NY Fed SCE + Dallas Trimmed Mean PCE + Cleveland inflation nowcast +
   SF Fed news sentiment + FDIC BankFind + CFPB complaints + IAPD + BrokerCheck.
3. **Financial conditions and macro stats:** FRED `fredgraph.csv` allowlist + ECB Data Portal +
   BIS + Eurostat + OECD CLI + IMF SDMX 3.0 CPI + UK ONS CPIH + OFR STFM + Chicago Fed NFCI +
   Philadelphia Fed ADS + Atlanta Fed GDPNow + NY Fed SCE + BoE/BoC/RBA/Norges/Riksbank/SNB +
   Bundesbank + StatsCan/ABS/SingStat/CBS/INSEE/SSB/StatFin/INE Spain + CSO Ireland + DST Denmark +
   Swiss FSO + Statistics Iceland + GUS Poland + IBGE + SCB Sweden + Statistics Estonia/Latvia/
   Slovenia/Slovakia.
4. **Global FX prints:** RBA + Norges Bank + Riksbank + SNB + BCB PTAX + NBP + CNB + Danmarks
   ...
   CBAR Azerbaijan + BNM Malaysia + TCMB Türkiye + BNB Bulgaria + BanRep Colombia (restored) +
   National Bank of Moldova (+ rediscovered HKMA ER when available).
...
8. **Cyber, physical, climate, and country risk:** CISA KEV + FIRST EPSS + NASA EONET + USGS
   volcanoes (`hans-public`) + NOAA PSL indices + NOAA CO-OPS + Open-Meteo + USGS NWIS IV +
   Climate TRACE + Climate Watch NDC + INFORM Risk + OpenFEMA + HDX allowlisted packages +
   OurAirports + VoteView + CourtListener watchlists.
9. **Crypto market structure (free):** DeFiLlama chain TVL + Coinbase/Kraken public market data +
   L2Beat scaling summary (+ Fear & Greed / mempool fees as P1 follow-ons).
...
and downloaded the response body on 2026-07-31 UTC (prior 2026-07-30/29 notes retained where still
accurate).

| Source | HTTP | Response type / observation |
|---|---:|---|
| Canada SEMA sanctions | 200 | `text/xml`; ~1.85 MB |
| UK Sanctions List CSV | 200 | HEAD OK; ~49.3 MB |
| GLEIF lei-records | 200 | JSON:API; ~3.39M LEIs in pagination meta |
| ESMA FIRDS Solr | 200 | JSON index; latest `FULINS_C_20260725_01of01.zip` |
| FINRA CNMS short volume (2026-07-30) | 200 | pipe text ~534 KB |
| TreasuryDirect auctioned | 200 | JSON |
| Bank of Canada Valet FXUSDCAD | 200 | JSON |
| FDIC institutions | 200 | JSON |
| Federal Register documents | 200 | JSON |
| BNM exchange-rate | 200 | JSON with Accept header |
| OpenFEMA DisasterDeclarationsSummaries | 200 | JSON |
| USGS monitored volcanoes (`hans-public`) | 200 | JSON; 70 volcanoes; WATCH/ORANGE present |
| NASA EONET events | 200 | JSON body (content-type still RSS-ish) |
| FRED `fredgraph.csv` DGS10/T10Y2Y/UNRATE/CPI/GDP *(new)* | 200 | CSV |
| CBOE `_VIX` / `_VVIX` / `_SKEW` history *(new)* | 200 | JSON ~0.6–1.2 MB |
| OCC volume-query 20260728 *(new)* | 200 | CSV ~5.0 MB / ~152k rows |
| UK ONS datasets + CPIH metadata/CSV *(new)* | 200 | JSON + CSV ~4.7 MB |
| IMF SDMX 3.0 structure dataflow *(recheck)* | 200 | JSON; ~222 dataflows |
| IMF SDMX 3.0 CPI data (sparse filters) | 404/403 | re-fixture full key path + Accept header; structure still OK |
| TCMB `today.xml` / dated XML *(new)* | 200 | XML ~9 KB; USD=47.3075 (29.07.2026) |
| BNB Bulgaria FX XML *(promoted)* | 200 | XML; USD/EUR table for 29.07.2026 |
| NOAA CO-OPS latest water level + stations *(new)* | 200 | JSON |
| Open-Meteo forecast + air quality *(new)* | 200 | JSON |
| USGS NWIS IV streamflow *(new)* | 200 | WaterML-JSON |
| NY Fed SCE workbook *(new)* | 200 | XLSX ~1.06 MB |
| OurAirports airports.csv *(new)* | 200 | CSV ~12.7 MB |
| data.gov.uk package_search *(new)* | 200 | CKAN JSON |
| WHO DON OData *(P1)* | 200 | JSON |
| OpenFEC DEMO_KEY *(P1)* | 200 | JSON |
| mindicador.cl dolar/UF *(P1)* | 200 | JSON |
| BanRep consultaMercadoCambiario (REST) | 200 | **JSON restored** (~63 KB tick array) |
| Destatis guest catalogue | 200 | HTML app shell — not usable JSON API |
| Swiss SECO sanctions | 200 | XHTML interstitial, not sanctions XML |
| MAS directory | 404 | portal HTML |
| Stats NZ open data root / opendata v1 | 404/502 | still unavailable |
| USGS `vsc/api/volcanoApi/*` | 404 | retired paths |
| IMF SDMX abbreviated version (`CPI/5.0`) | 404 | use full `5.0.0` |
| Chicago Fed CFNAI CSV *(new)* | 200 | CSV ~33 KB |
| Dallas Fed trimmed-mean PCE XLSX *(new)* | 200 | XLSX ~20 KB |
| Cleveland nowcast_month/quarter/year JSON *(new)* | 200 | chart JSON ~4.9–7.5 MB |
| SF Fed news_sentiment_data.xlsx / chart CSV *(new)* | 200 | XLSX ~417 KB; CSV ~276 KB |
| OTC Markets stock-screener API *(new)* | 200 | JSON; ~18,088 securities |
| Coinbase Exchange products/ticker/candles *(new)* | 200 | JSON |
| Kraken public Ticker/OHLC/AssetPairs *(new)* | 200 | JSON |
| L2Beat scaling/summary + activity *(new)* | 200 | JSON ~303 KB / ~1 KB |
| SEC IAPD firm/individual search *(new)* | 200 | JSON |
| FINRA BrokerCheck firm search *(new)* | 200 | JSON |
| CourtListener courts + search *(new)* | 200 | JSON |
| National Bank of Moldova FX XML *(new)* | 200 | XML ~7 KB with `date=` |
| CBOE delayed_quotes historical `_VIX` *(re-pin)* | 200 | JSON ~1.15 MB |
| CBOE `us_indices/daily_prices/_VIX` | 403 | S3 AccessDenied — avoid |
| GLEIF lei-records *(recheck)* | 200 | JSON:API; ~3.39M LEIs |
| OpenFIGI mapping AAPL *(recheck)* | 200 | JSON FIGI |
| Fear & Greed / mempool fees *(P1 recheck)* | 200 | JSON |
| ProPublica Congress | 401 | key required |
| AirNow observation | 401 | key required |
| WTO Stats | 401 | subscription key |
| UCDP GED API | 401 | token required |
