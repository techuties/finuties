...
**New in this review (2026-08-05):** NOAA GML greenhouse-gas trend text files (Mauna Loa CO₂
monthly/weekly/annual + global CH₄ / N₂O / SF₆), Coin Metrics Community API (`PriceUSD` and
catalog, no key), Aviation Weather Center METAR/SIGMET JSON (promoted from P1), Fraunhofer
Energy-Charts power/price JSON with explicit **CC BY 4.0 / SMARD** license_info (promoted),
crypto Fear & Greed + mempool.space fee API (promoted), and deps.dev package metadata
(companion to OSV #147). **Extensions:** multi-venue crypto #136 adds BitMEX, MEXC, HTX/Huobi,
Crypto.com Exchange, and dYdX v4 perpetual markets (GMX `/prices` also live). **Re-verified:**
GLEIF ~3,394,110 LEIs (golden copy 2026-08-05), UK Sanctions List report date **04-Aug-2026**,
CISA KEV catalogVersion **2026.08.04** (1,660 vulns), OTC Markets screener ~18,075, USDM
dataservices national CSV still map date 2026-07-28 + shapefile `USDM_20260728.*` + GeoJSON
~19.1 MB, NHC `activeStorms: []` (valid quiet), PortWatch chokepoints, FINRA CNMS 20260804
~540 KB, OCC volume-query 20260804 ~5.3 MB, BanRep `totoro` FX, NASA GISS, EMSC FDSN, NSIDC
v4 NH CSV, Hyperliquid `meta`, Pink Sheet / IMF PCPS workbooks, FRED `DGS10`, CBOE `_VIX.json`
timestamp 2026-08-05, ISO 10383 MIC CSV, OpenFIGI mapping, ESMA FIRDS Solr, Climate TRACE v6
country emissions. **Still blocked / watch:** CIRCL.lu anonymous burst → HTTP 429
(20/min — keep polite pacing on #158); ReliefWeb v1 returns **410 decommissioned**, v2 still
needs approved `appname` (403); GFW data-api SQL queries need free API key (catalog alone is
anonymous); USDM site root HTML paths vary — use `usdmdataservices.unl.edu` API +
`shapefiles_m/USDM_current_M.zip`; MarineCadastre AIS ZIP GETs still 404; Bybit country-blocked;
Binance.com geo-451 (use Binance.US); Banxico/KOSIS/FAS Open Data key-gated; Stats NZ OData 502
here; OpenSanctions full `entities.ftm.json` ~352 MB (prefer national lists; see P1 for
`targets.simple.csv` ~68 MB).
...
6. **Power and gas:** Elexon + ENTSOG + MIDAS + Energinet + Elia + REE Spain + ERCOT dashboards
   ...
   Energy-Charts DE power/price (#162) (+ AEMO only after terms clearance).
...
8. **Cyber, physical, climate, and country risk:** CISA KEV + FIRST EPSS + OSV.dev + CIRCL CVE +
   NASA EONET + NASA GISS + NOAA GML GHG trends (#159) + NSIDC sea ice + NOAA SWPC + NOAA PSL +
   NOAA CO-OPS + Aviation Weather METAR/SIGMET (#161) + NHC CurrentStorms/GeoRSS + US Drought
   Monitor (+ GeoJSON) + Open-Meteo (+ AQ/marine) + MET Norway + Bright Sky (DWD) + USGS NWIS IV +
   EMSC FDSN + USGS volcanoes + Climate TRACE + Climate Watch NDC + INFORM Risk + OpenFEMA +
   HDX allowlisted packages + IMF PortWatch chokepoints + OurAirports + VoteView + CourtListener
   watchlists + UN SDG + ILOSTAT labour allowlists.
9. **Crypto market structure (free):** DeFiLlama chain TVL + stablecoins + Coin Metrics
   ...
   Crypto.com / dYdX v4 public tickers + Fear & Greed / mempool fees (#163).
...
and downloaded the response body on **2026-08-05 UTC** (prior 2026-08-04/03/02/01/07-31 notes retained
where still accurate).

| Source | HTTP | Response type / observation |
|---|---:|---|
| NOAA GML `co2_mm_mlo.txt` *(new)* | 200 | text ~59 KB |
| NOAA GML `co2_weekly_mlo.txt` *(new)* | 200 | text ~209 KB |
| NOAA GML `ch4_mm_gl.txt` / `n2o_mm_gl.txt` / `sf6_mm_gl.txt` *(new)* | 200 | text ~46 / 28 / 32 KB |
| Coin Metrics Community `PriceUSD` BTC/ETH *(new)* | 200 | JSON timeseries + `next_page_token` |
| Coin Metrics `catalog/assets` *(new)* | 200 | JSON ~8.7 MB |
| Aviation Weather METAR multi-station *(promoted)* | 200 | JSON ~1.4 KB |
| Aviation Weather `airsigmet` *(promoted)* | 200 | JSON ~18 KB |
| Energy-Charts `public_power` / `price?bzn=DE-LU` *(promoted)* | 200 | JSON; price `license_info` CC BY 4.0 / SMARD |
| Fear & Greed `limit=5` *(promoted)* | 200 | JSON; latest value 27 (Fear) |
| mempool.space recommended fees *(promoted)* | 200 | JSON sat/vB ladder |
| deps.dev npm `lodash` *(promoted)* | 200 | JSON ~20 KB |
| BitMEX / MEXC / HTX / Crypto.com / dYdX v4 *(ext #136)* | 200 | JSON tickers / markets |
| GLEIF lei-records *(recheck)* | 200 | JSON:API; ~3,394,110 LEIs; golden copy 2026-08-05 |
| UK Sanctions List CSV *(recheck)* | 200 | CSV; Report Date **04-Aug-2026** |
| CISA KEV *(recheck)* | 200 | JSON; catalogVersion **2026.08.04**; count 1,660 |
| OTC Markets stock-screener *(recheck)* | 200 | JSON; count ~18,075 |
| USDM dataservices national area/percent *(recheck)* | 200 | CSV; map date still 2026-07-28 |
...
| FINRA CNMS 20260804 | 200 | pipe text ~540 KB |
| OCC volume-query 20260804 | 200 | CSV ~5.3 MB |
| BanRep `totoro` consultaMercadoCambiario | 200 | JSON tick array ~45 KB |
| NASA GISS `GLB.Ts+dSST.csv` | 200 | CSV ~13 KB |
| EMSC FDSN events | 200 | JSON FeatureCollection |
| NSIDC NH sea-ice extent v4 | 200 | CSV (partial) |
| Hyperliquid `meta` POST | 200 | JSON ~17.5 KB |
| World Bank Pink Sheet monthly/annual | 200 | XLSX |
| IMF `External_Data.xls` PCPS | 200 | OOXML XLSX despite `.xls` |
| FRED `fredgraph.csv?id=DGS10` | 200 | CSV ~268 KB |
| CBOE `_VIX.json` | 200 | JSON; timestamp 2026-08-05 |
| ISO 10383 MIC CSV | 200 | CSV |
| OpenFIGI mapping AAPL | 200 | JSON FIGI `BBG000B9XRY4` |
| ESMA FIRDS Solr FULINS | 200 | JSON |
| Climate TRACE v6 country emissions | 200 | JSON |
| OpenSanctions sanctions index | 200 | JSON; ~293k entities; `targets.simple.csv` ~68 MB |
| CIRCL.lu CVE-2024-3400 | 429 | Too Many Requests (`20 per 1 minute`) — pace #158 |
| ReliefWeb v1 disasters | 410 | API v1 decommissioned |
| ReliefWeb v2 disasters (unapproved appname) | 403 | approved `appname` still required |
| GFW umd_tree_cover_loss query | 403 | API key required for SQL query |
| Bybit v5 tickers | 403 | CloudFront country block |
| Banxico SIE without token | 400 | Token inválido |
| KOSIS without apiKey | 200 | JSON err KEY missing |
| Stats NZ opendata v1 | 502 | Azure gateway error here |
| USGS ScienceBase MCS cobalt `file/get` CSV | 404 | item JSON lists file; direct get failed — keep P1 |
