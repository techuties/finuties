...
downloads (XLS/CSV) remain P0 below because they need no API key.
...
**New in this review (2026-08-01):** LBMA precious-metal JSON (gold/silver/platinum/palladium),
NY Fed Global Supply Chain Pressure Index (XLS + CSV), Atlanta Fed Wage Growth Tracker workbook
(new `/Project/Atlanta/FRBA/...` media path), EIA no-key WPSR table CSVs + Brent/Henry Hub XLS,
National Bank of Georgia FX JSON, Banco Central do Brasil SGS series (SELIC/IPCA), NASDAQ Trader
trade-halts RSS, Census FT-900 `exh1.xlsx`, multi-venue crypto public tickers (Bitstamp, Gemini,
OKX, Bitfinex, KuCoin, Gate.io, Poloniex, Deribit), and ISO 10383 MIC CSV (promoted from P1).
Prior P0 anchors re-verified (Canada SEMA ~1.85 MB, UK Sanctions ~49.5 MB, GLEIF ~3.39M LEIs,
FRED `fredgraph.csv` DGS10/KCFSI, BoC Valet, BanRep Colombia JSON, FINRA CNMS 2026-07-31,
OCC `reportType=D` 2026-07-31 ~4.7 MB, OpenFIGI, ESMA FIRDS Solr, CBOE historical `_VIX`,
OTC Markets www screener ~18.1k, Coinbase/Kraken/L2Beat, USGS `hans-public`, OurAirports,
data.gov.uk). **OTC note:** `backend.otcmarkets.com` now 403 — use `www.otcmarkets.com/.../api`.
**SF Fed news:** `news_sentiment_data.xlsx` still 200; prior chart CSV path 404. **CBOE put/call
ratio CSVs** S3 AccessDenied — keep using historical VIX/VVIX/SKEW chart JSON only. Swiss SECO
still XHTML interstitial; Destatis guest still HTML shell; MAS directory still 404; Stats NZ /
RBNZ still unavailable; Binance public API still geo-451; Bybit CloudFront country-blocked here.
...
- **Implementation:** weekly WPSR CSV pull + daily/weekly XLS sync; checksum; parse STUB/date
  columns carefully; do not treat `ir.eia.gov/wpsr/overview.csv` or `ngs.csv` as free (403 here).
- **Terms/risk:** US government open data; still add attribution. Some `ir.eia.gov` paths are
  restricted — pin the verified table IDs.
...
2. **US markets microstructure & banking:** TreasuryDirect auctions + FINRA Reg SHO short volume +
   ...
   www screener + FRED `fredgraph.csv` allowlist + OFR + Chicago Fed NFCI/CFNAI + Philadelphia Fed
   ADS + Atlanta Fed GDPNow + Atlanta Fed Wage Growth + NY Fed SCE + NY Fed GSCPI + Dallas Trimmed
   Mean PCE + Cleveland inflation nowcast + SF Fed news sentiment + FDIC BankFind + CFPB
...
5. **Commodities, energy, housing, credit:** LBMA metals + EIA WPSR/Brent/HH files + FHFA HPI +
   Freddie Mac PMMS + UK Land Registry + CSO Ireland HPA02 + Census FT-900.
...
and downloaded the response body on 2026-08-01 UTC (prior 2026-07-31/30/29 notes retained where
still accurate).

| Source | HTTP | Response type / observation |
|---|---:|---|
| Canada SEMA sanctions | 200 | `text/xml`; ~1.85 MB |
| UK Sanctions List CSV | 200 | HEAD OK; ~49.5 MB |
| GLEIF lei-records | 200 | JSON:API; ~3.39M LEIs in pagination meta |
| ESMA FIRDS Solr | 200 | JSON index; `numFound` ~12.4k FULINS |
| FINRA CNMS short volume (2026-07-31) | 200 | pipe text ~534 KB |
| OCC volume-query `reportType=D` 20260731 | 200 | CSV ~4.7 MB (`quantity,underlying,...`) |
| TreasuryDirect auctioned | 200 | JSON |
| Bank of Canada Valet FXUSDCAD | 200 | JSON |
| BanRep consultaMercadoCambiario | 200 | JSON tick array (~38 KB on probe) |
| FRED `fredgraph.csv` DGS10 / KCFSI / FRBATLWGTUMHWGO | 200 | CSV |
| CBOE historical `_VIX` | 200 | JSON ~1.15 MB |
| CBOE put/call ratio CSVs | 403 | S3 AccessDenied — avoid |
| OTC Markets www screener | 200 | JSON; ~18,085 securities |
| OTC Markets `backend.otcmarkets.com` | 403 | prefer www host |
...
| USGS monitored volcanoes (`hans-public`) | 200 | JSON |
| OurAirports airports.csv | 200 | CSV ~12.7 MB |
| data.gov.uk package_search | 200 | CKAN JSON |
| Chicago Fed CFNAI / NFCI CSVs | 200 | CSV |
| Dallas Fed trimmed-mean PCE XLSX | 200 | XLSX ~20 KB |
| Cleveland nowcast_month JSON | 200 | chart JSON ~7.5 MB |
| SF Fed news_sentiment_data.xlsx | 200 | XLSX ~417 KB |
| SF Fed news-sentiment chart CSV (prior path) | 404 | prefer XLSX |
| LBMA gold_am / silver / platinum_am / palladium_am *(new)* | 200 | JSON ~0.55–0.92 MB each |
| NY Fed GSCPI xlsx + interactive CSV *(new)* | 200 | Excel ~182 KB; CSV ~219 KB |
| Atlanta Fed wage-growth-data.xlsx *(new path)* | 200 | XLSX ~334 KB |
| EIA WPSR table1/table2 CSV *(new)* | 200 | text/plain ~7–8 KB |
| EIA Brent / Henry Hub hist XLS *(new)* | 200 | Excel ~374–491 KB |
| NBG Georgia FX JSON *(new)* | 200 | JSON; USD=2.6266 on 2026-07-31 |
| BCB SGS Selic 432 / IPCA 433 *(new)* | 200 | JSON |
| NASDAQ tradehalts RSS *(new)* | 200 | RSS/XML ~36 KB |
| Census FT-900 exh1.xlsx *(new)* | 200 | XLSX ~246 KB |
| Bitstamp / Gemini / OKX / Bitfinex / KuCoin / Gate / Poloniex / Deribit *(new)* | 200 | JSON tickers |
| ISO 10383 MIC CSV *(promoted)* | 200 | CSV ~492 KB |
| National Bank of Moldova FX XML | 200 | XML ~7 KB with `date=31.07.2026` |
| Fear & Greed / mempool fees *(P1)* | 200 | JSON |
| mindicador.cl dolar *(P1)* | 200 | JSON |
...
| Stats NZ / RBNZ selected files | 404/403 | still unavailable |
| Binance public ticker | 451 | geo-restricted |
| Bybit public ticker | 403 | CloudFront country block |
| Banxico SIE without token | 400 | token required |
| Census ACS without key | 200 HTML | Missing Key page |
