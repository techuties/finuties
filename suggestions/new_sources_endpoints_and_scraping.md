...
**New in this review (2026-08-03):** US Drought Monitor Data Services (national + county CSV,
current shapefile ZIP), NOAA NHC `CurrentStorms.json` + Atlantic GeoRSS, EIA Short-Term Energy
Outlook workbook (`STEO_m.xlsx`) and Electric Power Monthly `table_1_01.xlsx` (extend #131),
ILOSTAT labour indicators via `rplumber.ilo.org` (TOC + curated indicator CSV; SDMX structure
live), and FRED `fredgraph.csv` allowlist expansions (`CSUSHPISA`, `UMCSENT`, `KCFSI`).
**Re-verified:** OTC Markets screener restored (~18,085 securities; prior day timeout cleared),
GLEIF ~3.39M LEIs (golden copy 2026-08-03), Canada SEMA ~1.85 MB, BanRep `totoro` FX ticks,
FINRA CNMS 20260730/20260731 (~534 KB; weekend `20260801` AccessDenied — expected), OCC
`reportType=D` 20260731 ~4.7 MB, PortWatch Daily_Chokepoints ArcGIS, UK Carbon Intensity,
OSV.dev package query, CISA KEV (1,656 vulns / catalog 2026.07.29), FIRST EPSS, LBMA gold_am,
BIS `WS_CBPOL`, Eurostat HICP, Gemini/Bitfinex/OKX/Deribit public tickers. **Still blocked /
avoid:** MarineCadastre AIS year indexes list ZIPs but file GETs 404 here — keep P2; CBOE
`us_indices/daily_prices/*` and PCRATIO CSV AccessDenied; Banxico/Ember/PJM/GovInfo key-gated;
Swiss SECO interstitial; Destatis guest HTML; MAS/Stats NZ/RBNZ unavailable; Binance geo-451.
...
### 131. EIA no-key petroleum, natural-gas, and outlook files *(extended 2026-08-03)*

- **Value:** free energy spot/stocks/outlook/generation prints without the gated EIA REST API
  key — complements the terminal energy card and commodities notebooks.
- **Entry points:**
...
- **Docs:** https://www.eia.gov/petroleum/supply/weekly/, https://www.eia.gov/outlooks/steo/,
  https://www.eia.gov/electricity/monthly/, and https://www.eia.gov/dnav/
- **Access:** no authentication for these file URLs (EIA REST `api.eia.gov` remains P3 key-gated).
- **Implementation:** weekly WPSR CSV pull + daily/weekly XLS sync + NGS history XLS + monthly
  STEO/EPM workbooks; checksum; parse STUB/date columns carefully; do not treat
  `ir.eia.gov/wpsr/overview.csv`, `ngs.csv`, or `ngs/overview.csv` as free (403 Access Restricted).
  Optional large companion (P1 cadence): `Region_US48.xlsx` from the EIA Grid Monitor known-issues
  path (~36 MB verified) — treat as bulk, not a hot path.
- **Terms/risk:** US government open data; still add attribution. Some `ir.eia.gov` paths are
  restricted — pin the verified table IDs / `ngshistory.xls` / `STEO_m.xlsx`.
...
### 148. US Drought Monitor Data Services *(new)*
...
- **Entry points:**
  - National area percent:
    `GET https://usdmdataservices.unl.edu/api/USStatistics/GetDroughtSeverityStatisticsByAreaPercent?aoi=us&statisticsType=1&startDate=8/1/2026&endDate=8/3/2026`
    (CSV; CONUS D0–D4 shares; verified 2026-08-03)
  - County area percent:
  ...
  - Current shapefile ZIP:
    ...
    `USDM_YYYYMMDD.*` members)
- **Docs:** https://droughtmonitor.unl.edu/DmData/DataDownload.aspx and
  https://usdmdataservices.unl.edu/
- **Access:** no authentication; CSV + ZIP shapefile.
- **Implementation:** weekly pull of national + selected-county series; upsert by `MapDate` /
  FIPS; archive shapefile checksums; do not invent interpolations between map dates.
- **Terms/risk:** USDM / NDMC attribution required; polygons are weekly snapshots.
...
5. **Commodities, energy, housing, credit:** LBMA metals + EIA WPSR/Brent/HH/NGS-history/STEO/
   Electric Power Monthly files + FHFA HPI + Freddie Mac PMMS + UK Land Registry + CSO Ireland
   HPA02 + Census FT-900 + FRED Case-Shiller/UMCSENT allowlist companions.
...
8. **Cyber, physical, climate, and country risk:** CISA KEV + FIRST EPSS + OSV.dev + NASA EONET +
   USGS volcanoes (`hans-public`) + NOAA PSL indices + NOAA CO-OPS + NHC CurrentStorms/GeoRSS +
   US Drought Monitor + Open-Meteo + MET Norway + Bright Sky (DWD) + USGS NWIS IV + Climate TRACE
   + Climate Watch NDC + INFORM Risk + OpenFEMA + HDX allowlisted packages + IMF PortWatch
   chokepoints + OurAirports + VoteView + CourtListener watchlists + UN SDG + ILOSTAT labour
   allowlists.
...
and downloaded the response body on **2026-08-03 UTC** (prior 2026-08-02/01/07-31 notes retained
where still accurate).

| Source | HTTP | Response type / observation |
|---|---:|---|
| US Drought Monitor national CSV *(new)* | 200 | CSV; CONUS D0–D4 area shares for map date 2026-07-28 |
| USDM current shapefile ZIP *(new)* | 200 | ZIP ~2.3 MB (`USDM_20260728.*`) |
| USDM Los Angeles County CSV *(new)* | 200 | CSV county severity rows |
| NHC `CurrentStorms.json` *(new)* | 200 | JSON; `activeStorms: []` (quiet day — valid) |
| NHC Atlantic GeoRSS *(new)* | 200 | XML/RSS ~2 KB |
| EIA `STEO_m.xlsx` *(new)* | 200 | XLSX ~1.1 MB |
| EIA Electric Power Monthly `table_1_01.xlsx` *(new)* | 200 | XLSX ~20 KB |
| EIA `Region_US48.xlsx` Grid Monitor *(P1 bulk)* | 200 | XLSX ~36 MB |
| EIA `ngshistory.xls` *(recheck)* | 200 | XLS ~730 KB |
| ILOSTAT plumber TOC *(new)* | 200 | CSV ~738 KB indicator catalog |
| ILOSTAT plumber `SDG_0111_SEX_AGE_RT_A` *(new)* | 200 | CSV ~6.9 MB labelled extract |
| ILOSTAT SDMX dataflow structure *(new)* | 200 | SDMX-ML ~7.3 MB |
| FRED `fredgraph.csv` CSUSHPISA / UMCSENT / KCFSI *(new)* | 200 | CSV |
| OTC Markets stock-screener *(restored)* | 200 | JSON; count ~18,085 / pages ~905 |
| GLEIF lei-records *(recheck)* | 200 | JSON:API; ~3,391,763 LEIs; golden copy 2026-08-03 |
| Canada SEMA sanctions HEAD | 200 | `text/xml`; ~1.85 MB |
| BanRep `totoro` consultaMercadoCambiario | 200 | JSON tick array ~38 KB |
| FINRA CNMS 20260730 / 20260731 | 200 | pipe text ~534 KB |
| FINRA CNMS 20260801 (Sun) | 403 | S3 AccessDenied — non-trading date |
| OCC volume-query 20260731 | 200 | CSV ~4.7 MB |
| IMF PortWatch Daily_Chokepoints ArcGIS | 200 | JSON features |
| UK Carbon Intensity | 200 | JSON half-hour intensity |
| OSV.dev query npm lodash | 200 | JSON vulns payload |
| CISA KEV | 200 | JSON; 1,656 vulns; catalogVersion 2026.07.29 |
| FIRST EPSS CVE-2024-3400 | 200 | JSON |
| NVD CVE 2.0 `resultsPerPage=1` *(P1)* | 200 | JSON; totalResults ~372,628 |
| LBMA gold_am | 200 | JSON ~922 KB |
| BIS WS_CBPOL US | 200 | SDMX CSV |
| ECB YC 3Y sample | 200 | SDMX CSV |
| Eurostat HICP EA | 200 | JSON-stat |
| Gemini / Bitfinex / OKX / Deribit tickers | 200 | JSON |
| MarineCadastre AIS 2024 index | 200 | HTML lists ZIPs |
| MarineCadastre AIS ZIP GET | 404 | listed object not downloadable here — P2 |
| Baker Hughes / ACAPS / Banxico / USDA NASS | fail/401 | remain P1/P3 as documented |
