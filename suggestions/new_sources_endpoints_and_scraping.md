...
| 5 | **BIS Statistics API v2** — [dataflow discovery](https://stats.bis.org/api/v2/structure/dataflow/all/all/latest) | Cross-border banking, global liquidity, credit, debt securities, property prices, effective exchange rates, and policy rates. High-value financial-stability coverage. | `/api/v1/data/economic/bis` | SDMX REST. Discover dataflows first, then implement small explicit series sets. Support CSV/JSON where offered and retain SDMX dimension codes. |
...
| [NIST NVD CVE API 2.0](https://services.nvd.nist.gov/rest/json/cves/2.0?resultsPerPage=1) | Direct without a key / JSON; 5 requests per rolling 30 seconds | Full CVE records, CVSS metrics, weaknesses, affected configurations, and references for broader cyber-risk enrichment. | Much larger and noisier than CISA KEV. Build an incremental modified-date sync and respect the low anonymous limit. |
| [openFDA enforcement reports](https://api.fda.gov/drug/enforcement.json?limit=1) | Direct anonymously / JSON; 1,000 requests per day per IP | Drug, device, and food recall/enforcement signals with firms, classification, geography, and dates. | US-only, safety disclaimers apply, and a free key is recommended for reliable production volume. |
...
| --- | --- | --- |
| **FRED API** | Free, but every API request requires a registered key; much of its content overlaps primary BLS, BEA, Treasury, and central-bank sources already used by FinUties. | Secret management exists and a curated set of FRED-only series is identified. |
| **US EIA API v2** | Free API key registration is mandatory. Bulk files need no key but are large and less convenient for incremental ingestion. | A service key and quota monitoring are available, or a narrow bulk-file adapter is justified. |
...
On 2026-07-16, unauthenticated requests returned HTTP 200 and structured data for all eight P0 entry points. HTTP 200 was also confirmed for the listed US Treasury Fiscal Data, Federal Register, NVD, openFDA, NOAA SWPC, geoBoundaries, Copernicus EMS, MeteoAlarm, USGS Water, OECD, HDX CKAN, OWID, and ECB examples. Endpoint availability is evidence of technical access, not blanket permission to redistribute the data.
