...
**New in this review (2026-08-04):** World Bank Pink Sheet CMO historical XLSX (monthly + annual),
IMF Primary Commodity Prices `External_Data.xls` (serves XLSX), NASA GISS GISTEMP global/zonal
CSV, NOAA SWPC space-weather JSON (Kp, NOAA scales, alerts, solar-cycle indices), EMSC
Seismic Portal FDSN event feeds, NSIDC Sea Ice Index daily extent v4 (NH/SH CSV), Hyperliquid
public `info` API (meta + allMids), and CIRCL.lu CVE 5.1 record API. **Extensions:** EIA
`dpr-data.xlsx` on #131; Open-Meteo air-quality + marine hosts on #111; DeFiLlama stablecoins
on #102; Binance.US public tickers/klines on #136 (geo-reachable unlike Binance.com);
USDM current GeoJSON (~19 MB) on #148 beside the shapefile ZIP. **Re-verified:** GLEIF
~3.39M LEIs (golden copy 2026-08-04), Canada SEMA ~1.85 MB, UK Sanctions List CSV ~49.5 MB
(report date 31-Jul-2026), OTC Markets screener ~18,081 / 18,081 pages, BanRep `totoro` FX,
FINRA CNMS 20260731 ~534 KB, OCC `reportType=D` 20260731 ~4.7 MB, PortWatch
`Daily_Chokepoints_Data` ArcGIS, USDM dataservices national CSV (map date 2026-07-28), NHC
`CurrentStorms.json` empty-active (valid), CISA KEV catalogVersion **2026.08.03** (count live),
OSV.dev npm lodash query, LBMA gold_am, BIS `WS_CBPOL`, ECB EXR, CBOE `_VIX.json`, TIC `mfh.txt`,
SIPRI Milex, UK Carbon Intensity + generation mix, MET Norway, Bright Sky, UN SDG Goal List,
ISO 10383 MIC CSV, Coinbase/Kraken tickers, NY Fed GSCPI XLSX, Atlanta Fed GDPNow XLSX,
FRED `fredgraph.csv?id=DGS10`. **Still blocked / avoid:** MarineCadastre AIS ZIP GETs 404;
CBOE VX futures historical CSV AccessDenied; FINRA TRACE guessed CDN paths AccessDenied;
abuse.ch URLHaus/MalwareBazaar/ThreatFox anonymous 401; BOK ECOS / Japan e-Stat key-gated;
Binance.com geo-451 (use Binance.US); Bybit CloudFront country-blocked; Swiss SECO
interstitial; Destatis guest HTML; MAS/Stats NZ/RBNZ unavailable; OPEC basket XML Cloudflare
403 here.
...
### 136. Multi-venue crypto public tickers *(extended 2026-08-04)*

- **Value:** credential-free spot/perp tickers across additional venues for cross-exchange crypto
  microstructure beyond Coinbase (#121), Kraken (#122), Hyperliquid (#157), and CoinGecko
  aggregates.
- **Entry points (verified 200 JSON on 2026-08-01; Binance.US re-verified 2026-08-04):**
...
- **Access:** no authentication; CSV + ZIP shapefile + GeoJSON.
- **Implementation:** weekly pull of national + selected-county series; upsert by `MapDate` /
  FIPS; archive shapefile/GeoJSON checksums; prefer dataservices CSV for area stats and shapefile
  or GeoJSON for polygons; do not invent interpolations between map dates.
...
| NVD CVE 2.0 | `https://services.nvd.nist.gov/rest/json/cves/2.0?resultsPerPage=1` | Anonymous low-volume JSON worked 2026-08-03 (~372k CVEs); still prefers API key / tight rate limits — after KEV+EPSS+OSV. |
...
2. **US markets microstructure & banking:** TreasuryDirect auctions + FINRA Reg SHO short volume +
   ...
   www screener + TIC major foreign holders + FRED `fredgraph.csv` allowlist + OFR +
...
5. **Commodities, energy, housing, credit:** World Bank Pink Sheet + IMF Primary Commodity Prices +
   ...
   UK Land Registry + CSO Ireland HPA02 + Census FT-900 + FRED Case-Shiller/UMCSENT allowlist
   companions.
...
8. **Cyber, physical, climate, and country risk:** CISA KEV + FIRST EPSS + OSV.dev + CIRCL CVE +
   NASA EONET + NASA GISS + NSIDC sea ice + NOAA SWPC + NOAA PSL + NOAA CO-OPS + NHC
   CurrentStorms/GeoRSS + US Drought Monitor (+ GeoJSON) + Open-Meteo (+ AQ/marine) + MET Norway +
   Bright Sky (DWD) + USGS NWIS IV + EMSC FDSN + USGS volcanoes + Climate TRACE + Climate Watch
   NDC + INFORM Risk + OpenFEMA + HDX allowlisted packages + IMF PortWatch chokepoints +
   OurAirports + VoteView + CourtListener watchlists + UN SDG + ILOSTAT labour allowlists.
9. **Crypto market structure (free):** DeFiLlama chain TVL + stablecoins + Coinbase/Kraken +
   Hyperliquid + L2Beat + Binance.US / Bitstamp / Gemini / OKX / Bitfinex / KuCoin / Gate /
   Poloniex / Deribit public tickers (+ Fear & Greed / mempool fees as P1 follow-ons).
...
and downloaded the response body on **2026-08-04 UTC** (prior 2026-08-03/02/01/07-31 notes retained
where still accurate).

| Source | HTTP | Response type / observation |
|---|---:|---|
| World Bank Pink Sheet monthly XLSX *(new)* | 200 | XLSX ~765 KB |
| World Bank Pink Sheet annual XLSX *(new)* | 200 | XLSX ~3.1 MB |
| IMF `External_Data.xls` PCPS *(new)* | 200 | OOXML XLSX ~480 KB (`.xls` suffix) |
| NASA GISS `GLB.Ts+dSST.csv` *(new)* | 200 | CSV ~13 KB |
| NASA GISS `ZonAnn.Ts+dSST.csv` *(new)* | 200 | CSV ~10 KB |
| NOAA SWPC planetary K-index *(new)* | 200 | JSON ~28 KB |
| NOAA SWPC NOAA scales *(new)* | 200 | JSON ~1 KB |
| NOAA SWPC alerts *(new)* | 200 | JSON ~51 KB |
| NOAA SWPC solar-cycle indices *(new)* | 200 | JSON ~512 KB |
| EMSC FDSN events JSON/text *(new)* | 200 | JSON FeatureCollection / TSV |
| NSIDC NH/SH sea-ice extent v4 *(new)* | 200 | CSV ~1.88 MB / ~1.82 MB |
| Hyperliquid `meta` / `allMids` POST *(new)* | 200 | JSON ~17.5 KB / ~15.7 KB (~944 mids) |
| CIRCL.lu CVE-2024-3400 *(new)* | 200 | JSON CVE_RECORD 5.1 ~12 KB |
| DeFiLlama stablecoins *(ext #102)* | 200 | JSON ~539 KB; ~413 pegged assets |
| Binance.US ticker/24hr/klines *(ext #136)* | 200 | JSON; BTCUSD last ~63675 |
| Open-Meteo AQ + marine *(ext #111)* | 200 | JSON hourly payloads |
| EIA `dpr-data.xlsx` *(ext #131)* | 200 | XLSX ~156 KB |
| EIA `wpsr/table9.csv` + `psw01.xls` *(ext #131)* | 200 | CSV ~47 KB / XLS ~1.3 MB |
| USDM dataservices national CSV *(recheck)* | 200 | CSV; map date 2026-07-28; D0–D4 shares |
| USDM current shapefile ZIP *(recheck)* | 200 | ZIP ~2.3 MB (`USDM_20260728.*`) |
| USDM current GeoJSON *(ext #148)* | 200 | GeoJSON ~19.1 MB |
| NHC `CurrentStorms.json` *(recheck)* | 200 | JSON; `activeStorms: []` (quiet — valid) |
| PortWatch `Daily_Chokepoints_Data` query | 200 | JSON features |
| GLEIF lei-records *(recheck)* | 200 | JSON:API; ~3,392,911 LEIs; golden copy 2026-08-04 |
| Canada SEMA sanctions *(recheck)* | 200 | XML ~1.85 MB |
| UK Sanctions List CSV *(recheck)* | 200 | CSV ~49.5 MB; Report Date 31-Jul-2026 |
| OTC Markets stock-screener *(recheck)* | 200 | JSON; count ~18,081 / pages ~18,081 at pageSize=1 |
| BanRep `totoro` consultaMercadoCambiario | 200 | JSON tick array ~47 KB |
| FINRA CNMS 20260731 | 200 | pipe text ~534 KB |
| OCC volume-query 20260731 | 200 | CSV ~4.7 MB |
| CISA KEV *(recheck)* | 200 | JSON; catalogVersion **2026.08.03** |
| OSV.dev query npm lodash | 200 | JSON vulns payload |
| CBOE `_VIX.json` historical charts | 200 | JSON ~1.2 MB; timestamp 2026-08-04 |
| LBMA gold_am | 200 | JSON ~922 KB |
| BIS WS_CBPOL US | 200 | SDMX CSV |
| ECB EXR USD/EUR sample | 200 | SDMX CSV |
| FRED `fredgraph.csv?id=DGS10` | 200 | CSV ~268 KB |
| TIC `mfh.txt` | 200 | text ~7.5 KB |
| SIPRI Milex XLSX | 200 | XLSX ~870 KB |
| NY Fed GSCPI XLSX | 200 | XLSX ~182 KB |
| Atlanta Fed GDPNow XLSX | 200 | XLSX ~10.8 MB |
| UK Carbon Intensity + generation | 200 | JSON |
| MET Norway locationforecast | 200 | JSON ~36 KB |
| Bright Sky current weather | 200 | JSON |
| UN SDG Goal List | 200 | JSON ~8 KB |
| ISO 10383 MIC CSV | 200 | CSV ~492 KB |
| Coinbase / Kraken BTC tickers | 200 | JSON |
| EIA `ngshistory.xls` / `STEO_m.xlsx` | 200 | XLS ~730 KB / XLSX ~1.1 MB |
| Berkeley Earth Land+Ocean *(P1)* | 200 | text ~417 KB |
| FSI-2023-DOWNLOAD.xlsx *(P1)* | 200 | XLSX ~27 KB |
| FSI-2024-DOWNLOAD.xlsx | 404 | not published at guessed path |
| CBOE VX futures historical CSV | 403 | S3 AccessDenied |
| FINRA TRACE guessed CDN path | 403 | S3 AccessDenied |
| abuse.ch URLHaus / MalwareBazaar / ThreatFox | 401 | Unauthorized without auth |
| BOK ECOS without service key | 200 | JSON ERROR-200 auth failure |
| Japan e-Stat without appId | 200 | STATUS 100 auth failure |
| MarineCadastre AIS ZIP GET | 404 | listed object not downloadable — keep P2 |
| Swiss SECO sanctions / Destatis guest / MAS | interstitial/HTML/404 | unchanged |
