...
**New in this review (2026-08-06):** SEC CNS fails-to-deliver half-month ZIPs (path
`/files/data/fails-deliver-data/`), Federal Reserve H.15 Data Download Program packages (Fed
funds/prime/discount + commercial paper + Treasury CMTs), CAISO real-time `demand.csv` /
`fuelsource.csv` (promoted; simpler than OASIS ZIP), openFDA enforcement/NDC/FAERS (promoted),
NASA POWER daily point meteorology (promoted), NASA DONKI CME/flare JSON, PSMSL tide-gauge RLR
files, CPC ENSO ONI / Niño SST index text, NOAA GHCN-Daily station CSVs, NHTSA vehicle recalls
(promoted), EPA eGRID 2023 workbook (`egrid2023_data_rev2.xlsx`), Blockchain.com Bitcoin network
stats/charts, and CPSC SaferProducts recalls JSON (promoted). **Re-verified:** GLEIF ~3,395,297
LEIs (golden copy 2026-08-06), UK Sanctions List report date **06-Aug-2026**, CISA KEV
catalogVersion **2026.08.05** (1,661), OTC Markets screener ~18,076, USDM shapefile/GeoJSON still
map date **2026-07-28** (dataservices national CSV path 404 here — prefer shapefile/GeoJSON),
NHC quiet-valid, PortWatch chokepoints, FINRA CNMS 20260805 ~539 KB, OCC volume-query 20260805
~5.1 MB, Hyperliquid `meta`, Pink Sheet / IMF PCPS, FRED `DGS10`, CBOE `_VIX.json` timestamp
2026-08-06, ISO 10383 MIC, Coin Metrics `PriceUSD`, Fear & Greed value **25** (Extreme Fear),
Energy-Charts DE power, deps.dev lodash, NOAA GML CO₂, OpenSanctions ~292k entities (updated
2026-08-06). **Still blocked / watch:** CIRCL.lu anonymous burst → HTTP 429 (20/min); ReliefWeb
v1 **410**, v2 approved `appname` still required; GFW SQL needs free API key; NASA FIRMS needs
`MAP_KEY`; USDA NASS Quick Stats unauthorized without key; Banxico/KOSIS/Ember/Electricity Maps
key-gated; CBOE `_MOVE.json` AccessDenied (keep VIX/VVIX/SKEW); Swiss SECO still XHTML
interstitial; Australia DFAT fetch failed here; MarineCadastre AIS ZIP GETs still 404; Bybit
country-blocked; Binance.com geo-451 (use Binance.US).
...
| OpenSanctions sanctions collection | `https://data.opensanctions.org/datasets/latest/sanctions/index.json` (+ `targets.simple.csv` ~68 MB; full `entities.ftm.json` ~352 MB on 2026-08-05) | Index live (~293k entities, updated 2026-08-05). Prefer Canada/UK/US/EU/UN official lists in P0 first; use `targets.simple.csv` only as a secondary cross-check with license review. |
...
2. **US markets microstructure & banking:** TreasuryDirect auctions + FINRA Reg SHO short volume +
   SEC CNS fails-to-deliver (#165) + OCC options volume (`reportType=D`) + CBOE VIX/VVIX/SKEW
   (historical charts path) + OTC Markets www screener + TIC major foreign holders + Fed H.15 DDP
   packages (#166) + FRED `fredgraph.csv` allowlist + OFR + Chicago Fed NFCI/CFNAI + Philadelphia
   Fed ADS + Atlanta Fed GDPNow + Atlanta Fed Wage Growth + NY Fed SCE + NY Fed GSCPI + Dallas
   Trimmed Mean PCE + Cleveland inflation nowcast + SF Fed news sentiment + FDIC BankFind + CFPB
   complaints + IAPD + BrokerCheck.
...
8. **Cyber, physical, climate, and country risk:** CISA KEV + FIRST EPSS + OSV.dev + CIRCL CVE +
   ...
   Aviation Weather METAR/SIGMET (#161) + NHC CurrentStorms/GeoRSS + US Drought Monitor (+ GeoJSON)
   + Open-Meteo (+ AQ/marine) + MET Norway + Bright Sky (DWD) + USGS NWIS IV + EMSC FDSN + USGS
   volcanoes + Climate TRACE + Climate Watch NDC + INFORM Risk + OpenFEMA + HDX allowlisted
   packages + IMF PortWatch chokepoints + OurAirports + VoteView + CourtListener watchlists +
   UN SDG + ILOSTAT labour allowlists.
...
and downloaded the response body on **2026-08-06 UTC** (prior 2026-08-05/04/03/02/01/07-31 notes
retained where still accurate).

| Source | HTTP | Response type / observation |
|---|---:|---|
| SEC `cnsfails202607a.zip` *(new)* | 200 | ZIP ~1.2 MB → pipe text; settlement 20260701+ |
...
| GLEIF lei-records *(recheck)* | 200 | JSON:API; ~3,395,297 LEIs; golden copy 2026-08-06 |
| UK Sanctions List CSV *(recheck)* | 200 | CSV; Report Date **06-Aug-2026** |
| CISA KEV *(recheck)* | 200 | JSON; catalogVersion **2026.08.05**; count 1,661 |
| OTC Markets stock-screener *(recheck)* | 200 | JSON; count ~18,076 |
| USDM dataservices national CSV *(recheck)* | 404 | prefer shapefile/GeoJSON paths |
| USDM current shapefile ZIP *(recheck)* | 200 | ZIP; members `USDM_20260728.*` |
| USDM current GeoJSON *(recheck)* | 200 | GeoJSON ~19.1 MB |
| NHC `CurrentStorms.json` *(recheck)* | 200 | JSON; `activeStorms: []` |
| PortWatch Daily_Chokepoints_Data *(recheck)* | 200 | JSON features |
| FINRA CNMS 20260805 | 200 | pipe text ~539 KB |
| OCC volume-query 20260805 | 200 | CSV ~5.1 MB |
