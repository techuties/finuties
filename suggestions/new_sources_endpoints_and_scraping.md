...
**New in this review (2026-08-31):** six newly verified credential-free
official prints, plus one existing-P0 count update. New P0s:
**OpenFEMA Public Assistance funded-project details**
(`/api/open/v2/PublicAssistanceFundedProjectsDetails`;
**848,204** projects; **1,329** `declarationDate` in 2026;
**25,916** `lastObligationDate` in 2026; fold applicants
**199,218**); **OpenFEMA Mission Assignments**
(`/api/open/v2/MissionAssignments`; **43,095** rows;
lastRefresh **2026-08-25**; live 2026 incidents including
Indiana flooding `2026081403` and Super Typhoon Sinlaku);
**CDC NHSN weekly hospital respiratory data** (`ua7e-t2fy`;
**21,172** rows; week ending **2026-08-22**; USA **1,935**
COVID / **565** flu inpatients; fold RESP-NET `kvib-3txy`
and RSV-NET `29hc-w46k`); **CDC Influenza A wastewater**
(`ymmh-divb`; **320,486** rows; latest
`sample_collect_date` **2026-08-25** — the live NWSS
companion while COVID `2ew6-ywp6` remains stale);
**NHTSA vehicle complaints** (`api.nhtsa.gov/complaints`;
Tesla Model 3 2023 **417** complaints, latest incident
**2026-08-22** filed **2026-08-27**; Model Y 2025 **96**);
and **NOAA NDBC latest marine observations**
(`latest_obs.txt`; **876** stations; LM **2026-08-31
08:05**). **Existing-P0 update:** NIH RePORTER FY2026
advanced to **52,859** (was 51,759). **Re-verified:**
FinUties `/health` online; `/api/v1/status/p0` **red** 0/8
this Monday morning (generated **2026-08-31T08:05:29Z**;
calendar age ~83h — weekend ingest lag, not a source
outage); GLEIF **3,416,456** LEIs (golden copy
**2026-08-30T16:00:00Z**; publishes **2026-08-31**
3,416,543 records); UK Sanctions List still
**28-Aug-2026** (LM **2026-08-28 09:39**); CISA KEV still
**2026.08.27** (**1,685**); FINRA CNMS **20260828** live
(~538 KB; 20260829 / 20260831 AccessDenied — Monday
morning not published yet); OCC **20260828** CSV live
(~4.62 MB); Fear & Greed **62** Greed; Coin Metrics BTC
**2026-08-30** ~77689; NY Fed SOFR still **2026-08-27**
3.64%; OpenSanctions ~291.6k (updated **2026-08-31**);
CFTC TFF/CIT still **2026-08-25**; PredictIt **186** /
SGX **899**; FEC `weball26.zip` LM **2026-08-31 05:42**
(~193 KB); DOL ETA 539 through **2026-08-22** (LM
**2026-08-30 19:51**); BTS border still **275,901**;
Senate LDA recovered (**1,976,496** filings); NSF Aug
**2,076** / YTD **6,466**; CDC NSSP still **651,372** /
`week_end` **2026-08-22**; House eFD **1,582** TSV lines
(LM **2026-08-28 13:00**); ClinicalTrials recruiting
**65,315** / 2026+ starts **32,469**; Storm Events stamp
still **c20260819**; DailyMed still **158,947**; RxNorm
still **03-Aug-2026**; SPC `today.csv` LM **2026-08-31
08:00** (107 lines); NIFC current incidents **522**;
FRED `DGS10` **2026-08-27** 4.67 / `SP500` **2026-08-28**
7711.76; NFIP v3 still **2,724,656**; NCHS week 33 still
**24,647** / `data_as_of` **2026-08-27**; BTS freight
still through **2026-08-25**. **Still blocked / watch:**
NCUA `call-report-data-2026-06.zip` still 404; `cbp24*`
still 404; QPC Q2 2026 still unpublished; FARS 2025 still
404; EIA-860M August 2026 still HTML redirect; CDC NWSS
COVID `2ew6-ywp6` latest `date_end` still **2025-09-07**
(stale — P1; use flu wastewater #310 instead);
`nasstatus.faa.gov` HTML **403** from this environment
this run (keep #303; pin `/api/` when reachable);
PHMSA `phmsa.dot.gov` HTML **403**; FRA `65fa-qbkf` live
but keep P1 (narrative PII); FDA `ndctext.zip` still fold
#168; OFAC `cons_prim.csv` still extend the US sanctions
adapter; NNDSS `x9gk-5huc` verified week **33** / **2026**
(**1,958,040** / **277,200** 2026 rows) — keep P1 one more
cycle beside NSSP/NHSN/NCHS.
...
- **Access:** no authentication; ZIP / XLSX. `august_generator2026.xlsx`
  and `xls/f923_2025.zip` still 301 to the electricity landing page
  here — pin the last 200 URL, not the landing-page redirect.
...
- **Implementation:** upsert `(weekendingdate, jurisdiction)`.
  ...
  same adapter on a faster cadence.
...
| NOAA NDBC `latest_obs.txt` | `https://www.ndbc.noaa.gov/data/latest_obs/latest_obs.txt` | **Promoted to P0 #312** (876 stations; LM 2026-08-31 08:05). |
| CDC NNDSS weekly `x9gk-5huc` | `https://data.cdc.gov/resource/x9gk-5huc.json` | Live (**1,958,040** rows; week **33** / **2026**; **277,200** 2026 rows; LM 2026-08-27). Keep P1 one cycle beside NSSP/NHSN/NCHS — promote when a notifiable-disease product is scoped. |
...
| OpenFEMA HMA disaster summaries v2 | `https://www.fema.gov/api/open/v2/HazardMitigationGrantProgramDisasterSummaries` | Live (**2,597**). HMA *projects* entity still 404 on v1/v2/v3 — keep P1 until the project worksheet path is pinned. |
...
| CDC NWSS wastewater | `https://data.cdc.gov/resource/2ew6-ywp6.json` | **837,382** rows live but latest `date_end` still **2025-09-07** (2026-08-31) — stale vs NSSP. Keep P1. **Influenza A wastewater `ymmh-divb` promoted to P0 #310** (collect through 2026-08-25). |
...
6. **Power and gas:** Elexon + ENTSOG + MIDAS + Energinet + Elia + REE Spain + RTE éco2mix (#192)
   ...
   July 2026 860M now live) + EIA Monthly Energy Review (#291) + BTS
...
8. **Cyber, physical, climate, and country risk:** CISA KEV + FIRST EPSS + OSV.dev + CIRCL CVE +
   ...
   CDC NHSN hospital respiratory (#309) + CDC Influenza A
   wastewater (#310) + NHTSA complaints (#311) + NOAA NDBC
   marine observations (#312) + IMF PortWatch
...
and downloaded the response body on **2026-08-31 UTC** (prior 2026-08-30/27/26/25/24/23/22/21/20/19/18/17/15/14/11/09/08/07/06/05/04/03/02/01/07-31
notes retained where still accurate). FinUties `GET /health` returned `{"status":"online"}` and
`GET /api/v1/status/p0` was **red** (0/8 domains green) at probe time — weekend ingest lag
(calendar age ~83h; last collection 2026-08-27 ~21:14Z), not an upstream-source outage.
