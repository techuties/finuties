...
/** Landing/catalog label; research categories carry an explicit suffix. */
export function categoryDisplayLabel(cat: CategoryDef): string {
  return isResearchCategory(cat.id) ? `${cat.label} (research)` : cat.label;
}

export const COMMERCIAL_EXPLORE_CATEGORIES = CATEGORIES.filter((c) => !isResearchCategory(c.id));
export const RESEARCH_EXPLORE_CATEGORIES = CATEGORIES.filter((c) => isResearchCategory(c.id));
